...
Prioritize Phase 1, but do not broaden packaged-data claims until Phase 2-quality inputs are available. The current draft path now passes the calibrated fit thresholds, adds a Moon longitude residual-correction channel when it improves the measured fit, and the target-threshold policy is recorded as production-ready, but the Phase 2 corpus still needs to be finalized; current artifact manifest, checksum, output-support, cadence, fit-outlier, and benchmark reports are useful diagnostics, not substitutes for release-grade evidence.
