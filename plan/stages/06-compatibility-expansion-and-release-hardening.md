...
- [x] The compatibility profile identifier has been bumped to `0.6.49`, the release-facing source-label appendix now also accepts `Sri Yukteshwar` alongside the existing Shri Yukteswar / Shri Yukteshwar forms while the Lahiri entry now also accepts `Chitra Paksha` / `Chitra-paksha` alongside `Chitrapaksha`, and the True Chitra entry now also accepts the exact `True Chitra ayanamsa` spelling so the broader transliteration batches stay synchronized across the profile, README, and resolver coverage.
