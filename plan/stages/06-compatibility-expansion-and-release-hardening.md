...
- [x] The release bundle display now surfaces the recorded validation-round count alongside the manifest path, making the release artifact summary a little more self-describing when maintainers verify or inspect a staged bundle. The README now mentions the same validation-round detail so the release-hardening docs stay aligned with the bundle output.
