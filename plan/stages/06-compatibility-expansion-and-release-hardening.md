...
- [x] `pleiades-core::ApiStabilityProfile` now explicitly lists `ChartSnapshot` motion summaries among the stable chart-facade helpers, keeping the consumer-facing posture synchronized with the chart helper surface.
...
- [x] The source-label appendix now also surfaces the PVR Pushya-paksha / True Pushya, J. N. Bhasin, Lahiri (VP285), Krishnamurti (VP291), Sheoran, remaining mean-sun / galactic-reference source forms, and the newer J2000/J1900/B1950, True Citra/Revati/Mula, Udayagiri, Lahiri (ICRC)/(1940), DeLuce, and Yukteshwar source spellings that users see in Swiss Ephemeris-style interoperability tables, keeping the release profile searchable by the labels that appear in upstream docs.
