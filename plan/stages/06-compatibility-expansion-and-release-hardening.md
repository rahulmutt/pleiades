...
- [x] The compatibility profile identifier has been bumped to `0.6.8` to reflect the new SS Revati/Citra catalog breadth, and the profile remains synchronized with the built-in ayanamsa catalog and validation output.
...
- [x] Catalog-alignment invariant tests now keep the built-in house-system and ayanamsa catalogs synchronized with the release compatibility profile, including the latest Valens Moon and SS Revati/Citra release-note coverage, and the profile identifier was bumped to `0.6.8`.
