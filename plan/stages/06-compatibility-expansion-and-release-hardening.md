...
- [x] The compatibility profile's source-label appendix now also includes `B. V. Raman -> Raman`, so the baseline Raman ayanamsa is searchable by the common author-name spelling, and the compatibility profile identifier has been bumped to `0.6.20` to keep the release artifact versioned with the latest appendix refinement.
