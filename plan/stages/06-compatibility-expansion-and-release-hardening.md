...
- [x] The compatibility profile identifier has been bumped to `0.6.48`, and the release-facing source-label appendix now also accepts `Sri Yukteshwar` alongside the existing Shri Yukteswar / Shri Yukteshwar forms while the Lahiri entry now also accepts `Chitra Paksha` / `Chitra-paksha` alongside `Chitrapaksha`, keeping the broader transliteration batches synchronized across the profile, README, and resolver coverage.
