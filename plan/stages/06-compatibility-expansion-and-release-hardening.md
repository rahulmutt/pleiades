...
- [x] The compatibility profile's source-label appendix now also includes `B. V. Raman`, `B.V. Raman`, and `B V Raman` source spellings for `Raman`, so the baseline Raman ayanamsa is searchable by the common author-name forms, and the compatibility profile identifier has been bumped to `0.6.21` to keep the release artifact versioned with the latest appendix refinement.
