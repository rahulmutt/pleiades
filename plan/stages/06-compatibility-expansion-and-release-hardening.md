...
- [x] The compatibility profile identifier has been bumped to `0.6.46`, and the release-facing source-label appendix now also accepts `Shri Yukteswar` and `Shri Yukteshwar` alongside the existing Yukteshwar / Sri Yukteswar forms, keeping the broad transliteration batch synchronized across the profile, README, and resolver coverage.
