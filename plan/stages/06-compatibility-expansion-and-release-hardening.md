...
- [x] The compatibility profile identifier has been bumped to `0.6.52`, and the stage-4 validation corpus now renders as a separate validation-reference-point section so the known-gaps list stays focused on actual compatibility gaps while the release-facing profile and validation summary continue to surface the reference point explicitly.
