...
- Completed: the packaged-artifact phase-2 corpus alignment summary now includes the source/provenance lines for the reference, comparison, and hold-out corpora alongside the body-class coverage evidence, and now also keeps the selected-asteroid source evidence/windows and the production-generation source provenance in the same phase-2 posture, keeping the threshold posture aligned with the recorded corpus metadata and the separate generation-input corpus.
