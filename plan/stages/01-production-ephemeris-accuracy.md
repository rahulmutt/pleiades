...
2. Implement production `pleiades-elp` lunar calculations.
   ...
   - Progress note: the lunar reference evidence summary now also carries the JD 2448724.5..2459278.5 range explicitly, making the current Moon-example-to-2021 coverage window visible in release-facing summaries.
   ...
   - Progress note: the lunar reference slice now also includes the 1992 Moon example, J2000 lunar-point anchors, and a published 2021-03-05 mean-perigee example, which broadens the current validation evidence without changing the public request model.
...
5. Expand validation evidence.
   ...
   - Progress note: the compact release summary now also carries a short validation evidence line with comparison sample and tolerance counts, so the release-facing overview exposes a little more of the current error envelope without expanding into the full validation report.
