...
5. Integrate catalog evidence with release profiles.
   ...
   - Progress note: the compatibility-profile verification now also rejects case-insensitive duplicate labels that would collide across different house-system, ayanamsa, or custom-definition entries, and the built-in Vehlow house-system descriptors were cleaned to avoid a case-only alias collision, so future catalog additions cannot silently drift into ambiguous cross-entry alias collisions even when spellings differ only by ASCII case.
