...
- Completed: the compatibility-caveats release surface now routes through a shared core formatter and cached validate renderer, so the caveat inventory shares the same fail-closed path before release-facing text is emitted.
