...
- preliminary `pleiades-vsop87`, `pleiades-elp`, and `pleiades-data` crates, including finite-difference mean-motion estimates in the VSOP87 path, a first truncated IMCCE VSOP87B Earth coefficient slice for J2000-tested geocentric Sun output, explicit geocentric-only rejection of direct topocentric requests in the VSOP87/ELP placeholder backends, plus a small JPL Horizons derivative-fixture backend with exact lookup and linear interpolation proof of concept;
...
1. replace the checked-in truncated Earth coefficient slice with a reproducible generated complete-table path;
2. add at least one non-Sun planetary body/channel using the same coefficient representation;
