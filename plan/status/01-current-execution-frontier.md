...
The reference corpus also now includes a 1749 major-body boundary slice, a 1750-01-01 interior selected-body slice, dedicated 2451913.5 and 2451917.5 boundary-day reports, and a dedicated 2451920.5 interior report surface, so the next reviewable step can move to one more reference-breadth slice or a clearly scoped request-policy doc/report cleanup if no additional corpus row is needed; the top-level reference snapshot summary now mirrors those slices as well.
