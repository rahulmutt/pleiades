...
1. **Artifact fit quality** — current fit envelope improved substantially after the Moon and recursive cubic-window slices, but the remaining high-error bodies still need to come down before production sign-off.
...
Status update: the Moon residual-correction slice is complete, the adjacent cubic-window slice is complete, the cadence-aware span-cap slice is complete, the manifest-completeness slice is complete (encoded artifact size and residual-body coverage are now explicit in the generator parameters), the current draft thresholds are calibrated to the latest artifact, the main benchmark report now includes packaged-artifact lookup, batch lookup, and decode sections, and the artifact summary now adds both a channel-major fit-outlier view and a body-class cadence view. Next work should still target the remaining high-error body classes or segment families.
