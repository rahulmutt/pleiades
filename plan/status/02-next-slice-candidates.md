...
- The 2200 selected-body boundary slice now also has a 2524593 JD-labeled alias in the CLI and validation front ends, and the 2500 selected-body boundary slice now also has a 2634167 JD-labeled alias, so that pair of frontier points is fully covered by both year and epoch naming.
...
- The exact-J2000 evidence summary is now also surfaced through the top-level reference snapshot summary, and the selected-asteroid boundary, bridge, dense, and terminal evidence slices are now surfaced there as well, so the next breadth slice can focus on another boundary epoch or representative interior row if more reference breadth is still needed.
