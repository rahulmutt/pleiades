...
Progress note (2026-04-26): the VSOP87 source-documentation summary now also carries explicit generated-binary, vendored full-file, and truncated body lists, so the reproducibility line can name the exact body coverage behind each source-path category instead of only reporting counts.
