...
- The lunar boundary evidence summary is now also surfaced through the top-level reference snapshot summary, so the next breadth slice can focus on another boundary epoch or a representative interior comparison row.
...
- The 2451915.25/2451915.75 high-curvature hold-out window is now surfaced through the combined JPL evidence report, and the selected-asteroid boundary, bridge, and dense summaries are now surfaced there too, while the 2451910 through 2451915 major-body boundary summaries are now also explicitly surfaced there; keep hold-out rows separate from fitting/reference rows while the next slice targets any remaining boundary breadth.
