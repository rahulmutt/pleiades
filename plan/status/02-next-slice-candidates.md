...
- Completed: the packaged-artifact source-fit and hold-out sync summary now exposes a validated CLI surface that couples the calibrated fit thresholds with the current target-threshold posture, so the source-fit checks can be reviewed alongside the hold-out-aligned release policy.
