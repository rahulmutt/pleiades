...
Progress note (2026-04-26): the VSOP87 source-documentation summary now also carries explicit generated-binary, vendored full-file, and truncated body lists, so the reproducibility line can name the exact body coverage behind each source-path category instead of only reporting counts. The validation, backend-matrix, and release summaries now also render a VSOP87 source-documentation health line, which makes the catalog partition check visible alongside the source-path mix.
...
Progress note (2026-04-26): the compact time-scale policy wording is now backend-owned and reused by validation/release reporting, so the current TT/TDB and caller-supplied conversion posture stays synchronized across the compact report surfaces without another crate-local literal.
...
Progress note (2026-04-26): the compact compatibility-profile, release-notes summary, and release-summary smoke tests now pin the exact custom-definition label count and compatibility-caveat count, so the short release-facing profile views stay anchored to the current known-gap totals instead of only asserting that those lines exist.
...
Progress note (2026-04-26): the compatibility-profile command tests now also pin the `Equal (cusp 1 = Asc)` and `Whole Sign (house 1 = Aries)` forms, so the ascendant-anchored equal-house and whole-sign source-label wording stays anchored in the release-facing verification path. The compact compatibility-profile summary and verification output now also label the release-specific house-system and ayanamsa canonical-name lists explicitly, so the short release-facing profile audit makes those canonical slices visible instead of implying them only through the rendered entries. The same CLI release-profile slice now also pins `Poli-equatorial`, `horizon/azimuth`, `T Polich/Page ("topocentric")`, `Zariel`, and `Equal/MC = 10th`, so a few more high-signal house-label spellings stay anchored in the command-level audit path.
