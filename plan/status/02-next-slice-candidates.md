...
Progress note (2026-04-24): the VSOP87 source-data path now covers the Sun through Neptune with vendored full-file IMCCE/CELMECH VSOP87B inputs, exact J2000 tests, and validation evidence for all source-backed major planets. The remaining Phase 1 work for this track is reproducible generated-table ingestion and release-grade error envelopes.
...
Progress note (2026-04-24): the VSOP87 validation output now also pairs each source-backed body profile with its source file, provenance, and measured canonical deltas, and the compact summary now reports a body-profile evidence count. The release-facing validation summaries now also distinguish vendored full-file profiles from the Pluto mean-element fallback, which makes the current source state clearer while the generated complete-table path remains pending.
