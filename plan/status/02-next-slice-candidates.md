...
- Completed: the packaged-artifact phase-2 corpus alignment posture now has a standalone validated CLI summary surface, and the regenerate-packaged-artifact regression now batches its sidecar writes so the slow fixture rewrite stays tractable under the test harness.
