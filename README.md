...
- Current first-party backends expose mean geometric coordinates only; `--apparent` is parsed for API compatibility but returns an unsupported-request error until apparent-place corrections are implemented. The chart CLI also keeps UTC/UT1 conversion explicit via `--tt-offset-seconds`, `--tt-from-utc-offset-seconds`, and `--tt-from-ut1-offset-seconds`. See [docs/time-observer-policy.md](docs/time-observer-policy.md).
