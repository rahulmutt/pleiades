...
- `ChartSnapshot` now offers lookup helpers for body, sign, house, sign-scoped placement, and house-scoped placement questions plus direct/retrograde motion helpers, motion summaries, and aspect summaries so downstream chart reports can ask higher-level questions without re-scanning placements manually,
...
- the release compatibility profile and API stability posture are both published through `pleiades-core`, surfaced in the CLI and validation reports, and kept in sync with the current release notes bundle and expanded source-label appendix,
