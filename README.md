...
- a release compatibility profile (`pleiades-compatibility-profile/0.7.12`),
...
- planetary nodes and apsides (SP-4) via `pleiades-events`: `EventEngine::nod_aps`/`nod_aps_default` (`swe_nod_aps` analogues) compute ascending-node and apsis (periapsis/apoapsis) points and distances via three methods — Mean, Osculating, and OsculatingBarycentric — with a caller-selected `ApsisConvention` of Aphelion or SecondFocus (the empty focus at 2ae); `pleiades-apsides` is generalized from its prior true-lunar-apside-only surface into shared `elements_from_state`/`points_from_elements` Kepler-elements helpers reused by both the lunar-apsides path and the new engine. The Mean method serves the Moon, Sun, and Mercury–Neptune (Earth is not addressable; the Sun's node/apsis slots are zeroed, matching Swiss Ephemeris's own `swe_nod_aps` behavior); Osculating/OsculatingBarycentric serve any body the backend chain can supply a state vector for, including the SP-3 fictitious bodies and packaged asteroids. Gated by the fail-closed `validate-nod-aps` gate (aliases `validate-nod-aps`/`nod-aps-gate`) over a committed 184-row Swiss-Ephemeris corpus (checksum-guarded, pinned by row count; the 20 barycentric reference rows are SWIEPH/DE431-based since Moshier cannot produce barycentric positions, all other rows MOSEPH). Measured accuracy: MEAN_PLANET and MEAN_MOON both sub-arcsecond (max longitude 0.658″ and 0.561″ respectively); OSCU_PLANET is arcminute-class (max longitude 1415″, a heliocentric Neptune perihelion row where Neptune's small eccentricity e≈0.0086 divides a legitimate ~arcsecond cross-ephemeris state difference by e — the node on those same rows still agrees to ~7″); OSCU_MOON is cross-theory (max longitude 3402″, apse-concentrated, nodes ≤18″, speed max 2.634°/day). Known coverage bound: `swe_nod_aps` does not implement fictitious bodies upstream and offline backend chains cannot supply the continuous sub-day state sampling osculating small-body nodes/apsides require, so fictitious/asteroid `nod_aps` is engine-covered but gate-unreferenced.
- phase, phase angle, and magnitude (SP-5) via `pleiades-events`: `EventEngine::pheno` (a `swe_pheno` analogue) computes `phase_angle_deg`, `phase_fraction` (illuminated fraction), `elongation_deg`, `apparent_diameter_deg`, and `apparent_magnitude: Option<f64>` for a body's geocentric apparent-of-date phenomena. Full five-output parity is served for the ten majors (Sun, Moon, Mercury–Pluto); other backend-served bodies (asteroids, SP-3 fictitious bodies) get the four geometric outputs with `apparent_magnitude = None` — magnitude coverage is a documented bound to the ten majors, and that path is gate-unreferenced. Two deliberate, measured (not silent) deviations from SE: `apparent_diameter_deg` is in **degrees**, matching SE's raw `attr[3]` slot, not arcsec; the Sun's `phase_angle_deg`/`phase_fraction`/`elongation_deg` are all zero, matching Swiss Ephemeris's own `swe_pheno` behavior of leaving those attributes unset for the Sun. Gated by the fail-closed `validate-pheno` gate (aliases `validate-pheno`/`pheno-gate`), wired into `run_all_numeric_gates`, over a committed 80-row Swiss-Ephemeris Moshier (`SEFLG_MOSEPH|SEFLG_NOGDEFL`, iflag 516) corpus (10 majors × 8 epochs, 1900–2100 CE), checksum-guarded and pinned by row count. Measured accuracy: phase angle holds to 85.0″ (max 57.79″, Mercury); elongation holds to 30.0″ (max 20.97″, Uranus); illuminated fraction holds to 2e-4 (max 1.2e-4); apparent diameter holds to 0.3″ (max 0.19″, Moon); apparent magnitude holds to 0.004 for all majors except Saturn (max 0.0023, Mercury), with a separate Saturn carve-out at 0.0006 (max 0.0004) since Saturn's ring term is the widest contributor to the magnitude residual.
- lunar occultations (SP-6) via `pleiades-events`: `EventEngine::occultation` (local circumstances for a given observer — the local-circumstances "how" analogue; Swiss Ephemeris has no separate `swe_lun_occult_how` call, so this is validated against the `attr` that `swe_lun_occult_when_loc` returns), `EventEngine::next_occultation`/`previous_occultation` (per-observer search, `swe_lun_occult_when_loc` analogues), and `EventEngine::next_global_occultation` (global search, a `swe_lun_occult_when_glob` analogue) for the Moon occulting planets Mercury–Pluto and a curated fixed-star catalog. `next_global_occultation` reports a single **central-observation point** — the geographic point where topocentric Moon–target separation is minimized — **not** the full central-path polygon; central-path cartography stays out of scope. Gated by the fail-closed `validate-occultations` gate (CLI aliases `validate-occultations`/`occultations`/`occult-gate`), wired into `run_all_numeric_gates`, over a committed 62-row Swiss-Ephemeris reference corpus (checksum-guarded, pinned by row count). Measured accuracy: contact/maximum instants hold to 65.0 s for well-conditioned rows (max 46.44 s), widening to 995.0 s near grazing/near-tangent limb geometry (max 710.03 s — an expected consequence of a near-zero closing rate perpendicular to the limb amplifying a small positional difference into a larger timing one, not a bug); star magnitude and obscuration are **exact** SE parity (max 0.0, pinned to a 1e-6 float-noise floor); planet magnitude holds to a 7% **relative** ceiling (max 4.89%); the global sub-lunar (central-observation) point holds to 30.0′ (max 20.22′, Antares); planet-grazing obscuration holds to a 7% relative ceiling (max 4.93%). Two known bounds are deliberately **not** gated: planet-**total** obscuration (SE's `attr[2]` for a fully-covered planet is a different, coverage-depth quantity that a bounded `[0,1]` area fraction cannot and should not reach — the engine's value, not SE's, is the physically correct obscuration), and the planet `central` boolean flag (measured but not gated — a narrow-margin discrepancy on 2 of 6 Saturn global rows, diagnosed as a conceptual difference from SE's axis-pierce central test rather than a positional error; a bounded future item). Near the graze limit at high geographic latitude, Total/Miss classification may disagree with SE by up to ~12′ of track width (8 of 18 measured geometric-miss rows, regression-pinned via `MAX_MISS_CLASSIFICATION_DISAGREEMENTS`, root cause a follow-up); deep-total and clear-miss cases agree with SE closely.
