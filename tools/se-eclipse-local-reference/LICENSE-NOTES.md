# License notes — `se-eclipse-local-reference`
...
to **generate a reference corpus of local (per-observer) solar and lunar eclipse
circumstances** — contact instants, magnitude/obscuration, and horizontal
(azimuth / altitude) coordinates — used to validate the pure-Rust
`pleiades-eclipse` local layer. It runs the Moshier ephemeris (`SEFLG_MOSEPH`),
so no Swiss Ephemeris `.se1` data files are bundled, read, or distributed.
...
Ephemeris license terms for their use. See the sibling `se-rise-trans-reference`
