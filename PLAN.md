...
- `pleiades-compression` and `pleiades-data` with codec validation, profile metadata, checksums, residual support, a deterministic prototype artifact, regeneration helpers, packaged lookup behavior, and explicit lookup-epoch policy metadata in the production-profile draft; the checked-in fixture now tracks the expanded reference-snapshot slice, but the artifact is not yet a production 1500-2500 CE data product;
