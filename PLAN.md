...
- `pleiades-compression` and `pleiades-data` codec/profile/checksum support plus a stage-5 draft packaged-data artifact for the 1500-2500 CE range; the packaged artifact has recently been retuned to same-body quadratic windows with longitude unwrapping across sampled epochs, materially improving the fit envelope and reducing artifact size while remaining draft-grade; the artifact reports now also surface a channel-major fit-outlier view to help prioritize the next fit slices;
...
| --- | --- | --- |
| 1. Production compressed data | Active | Highest leverage next work: continue reducing the remaining high-error bodies after the quadratic-window slice; the new channel-major fit-outlier view should help split cadence-driven follow-on work, then tighten thresholds and benchmarks. |
