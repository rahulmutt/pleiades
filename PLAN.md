...
- `pleiades-compression` and `pleiades-data` codec/profile/checksum support plus a stage-5 draft packaged-data artifact for the 1500-2500 CE range; the packaged artifact has recently been retuned to recursively subdivided cubic windows with longitude unwrapping and body-class span caps, materially improving the fit envelope and body-class cadence reporting while remaining draft-grade; the artifact reports now also surface channel-major fit-outlier and body-class cadence views to help prioritize the next fit slices;
...
| --- | --- | --- |
| 1. Production compressed data | Active | Highest leverage next work: continue reducing the remaining high-error bodies after the cubic-window slice; the new channel-major fit-outlier and body-class cadence views should help split cadence-driven follow-on work, then tighten thresholds and benchmarks. |
