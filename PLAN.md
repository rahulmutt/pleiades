...
- the release summary now surfaces validated production-generation body-class coverage and corpus-shape lines alongside the release-grade body-claims line, keeping the top-level release overview tied to corpus evidence; the same corpus-derived body/date/channel claims posture now also has a standalone `body-date-channel-claims-summary` / `body-date-channel-claims` surface for direct inspection;
