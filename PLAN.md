...
- The `pleiades-cli` release-bundle smoke tests now also pin `workspace-audit-summary.txt` alongside `native-dependency-audit-summary.txt`, and the bundled help-text coverage now explicitly exercises the workspace/native-dependency audit summary entries in the release-bundle overview.
