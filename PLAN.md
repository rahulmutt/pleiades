...
- `pleiades-data` ships a small deterministic prototype compressed artifact with codec validation, regeneration helpers, checksums, summaries, benchmark evidence, and a structured production-profile / target-threshold skeleton; the checked-in fixture is regenerated from the updated reference snapshot, but it is not yet a 1500-2500 CE production artifact with acceptable measured fit error.
