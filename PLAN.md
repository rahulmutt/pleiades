...
- `pleiades-data` ships an ARTIFACT_VERSION 6 compressed artifact (SP2
  heliocentric-planet reframe complete, all bodies sub-arcsec); published
  production thresholds and size/latency budgets (SP3) are not yet defined.
...
reference corpus and is window-independent. ARTIFACT_VERSION 6.
...
remain sub-arcsec). SP3 (thresholds/budgets) is next.
