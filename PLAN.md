...
- `pleiades-compression` and `pleiades-data` with codec validation, profile metadata, checksums, residual support, a deterministic prototype artifact, regeneration helpers, and packaged lookup behavior; the checked-in fixture now tracks the expanded reference-snapshot slice, but the artifact is not yet a production 1500-2500 CE data product;
