...
- `pleiades-compression` and `pleiades-data` codec/profile/checksum support plus a stage-5 draft packaged-data artifact for the 1500-2500 CE range; the packaged artifact has recently been retuned to adjacent linear spans with longitude unwrapping across sampled epochs, materially improving the fit envelope and reducing artifact size while remaining draft-grade;
...
| --- | --- | --- |
| 1. Production compressed data | Active | Highest leverage next work: continue reducing the remaining high-error bodies after the linear-span slice, then tighten thresholds and benchmarks. |
