...
mod tests {
    ...
    fn reference_snapshot_covers_the_expected_bodies_and_epochs() {
        ...
        assert_eq!(reference_epochs().len(), 7);
        assert_eq!(
            reference_snapshot()
                .iter()
    ...
    fn production_generation_snapshot_summary_reports_the_boundary_overlay() {
        ...
        assert_eq!(summary.row_count, 97);
        assert_eq!(summary.body_count, 15);
        assert_eq!(summary.bodies, reference_bodies());
        assert_eq!(summary.epoch_count, 9);
    ...
    fn reference_snapshot_batch_parity_summary_reports_the_expected_coverage() {
        ...
        assert_eq!(
            summary.summary_line(),
            format!(
                "JPL reference snapshot batch parity: 63 rows across 15 bodies and 7 epochs (JD 2378499.0 (TDB)..JD 2634167.0 (TDB)); bodies: {}; frame mix: 32 ecliptic, 31 equatorial; quality counts: Exact=63, Interpolated=0, Approximate=0, Unknown=0; batch/single parity preserved",
                format_bodies(reference_bodies())
            )
        );
        assert_eq!(summary.to_string(), summary.summary_line());
        ...
        assert!(jpl_snapshot_evidence_summary_for_report().contains(
            "JPL reference snapshot batch parity: 63 rows across 15 bodies and 7 epochs (JD 2378499.0 (TDB)..JD 2634167.0 (TDB)); bodies:"
        ));
    ...
    fn comparison_snapshot_summary_reports_the_expected_coverage() {
        ...
        assert_eq!(summary.row_count, 43);
        assert_eq!(summary.body_count, 10);
        ...
        assert_eq!(
            summary.summary_line(),
            "Comparison snapshot coverage: 43 rows across 10 bodies and 7 epochs (JD 2378499.0 (TDB)..JD 2634167.0 (TDB)); bodies: Mars, Mercury, Moon, Sun, Venus, Jupiter, Saturn, Uranus, Neptune, Pluto"
        );
        assert_eq!(summary.to_string(), summary.summary_line());
        assert_eq!(
            comparison_snapshot_summary_for_report(),
    ...
    fn comparison_snapshot_batch_parity_summary_reports_the_expected_coverage() {
        ...
        assert_eq!(summary.snapshot.row_count, 43);
        ...
        assert_eq!(summary.ecliptic_request_count, 22);
        assert_eq!(summary.equatorial_request_count, 21);
        assert_eq!(summary.validate(), Ok(()));
        assert_eq!(summary.validated_summary_line(), Ok(summary.summary_line()));
        assert_eq!(
            summary.summary_line(),
            format!(
                "JPL comparison snapshot batch parity: 43 rows across 10 bodies and 7 epochs (JD 2378499.0 (TDB)..JD 2634167.0 (TDB)); bodies: {}; frame mix: 22 ecliptic, 21 equatorial; quality counts: Exact=43, Interpolated=0, Approximate=0, Unknown=0; batch/single parity preserved",
    ...
    fn reference_snapshot_manifest_parses_the_documented_header_comments() {
        ...
        assert_eq!(manifest.coverage.as_deref(), Some("inner planets sampled across 1800-2500, with an additional 2406 Mars hold-out; outer planets and Pluto sampled at 2400000, 2451545, and 2500000; selected asteroids sampled at J2000, 2001-01-01, 2132-08-31, and 2500-01-01."));
        assert_eq!(
            manifest.columns,
        ...
        assert_eq!(
            ...
            "Reference snapshot manifest: JPL Horizons reference snapshot.; source=NASA/JPL Horizons API, DE441, geocentric ecliptic J2000 vector tables.; coverage=inner planets sampled across 1800-2500, with an additional 2406 Mars hold-out; outer planets and Pluto sampled at 2400000, 2451545, and 2500000; selected asteroids sampled at J2000, 2001-01-01, 2132-08-31, and 2500-01-01.; columns=epoch_jd, body, x_km, y_km, z_km"
        );
    }
    ...
    fn interpolation_quality_summary_reports_the_worst_case_labels() {
        ...
        assert!(rendered.contains("23 samples across 10 bodies and 5 epochs"));
    ...
    fn interpolation_quality_kind_coverage_reports_the_distinct_body_breakdown() {
        ...
        assert_eq!(coverage.sample_count, 23);
        ...
        assert!(rendered.contains("23 samples across 10 bodies ["));
