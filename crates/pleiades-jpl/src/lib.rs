...
mod tests {
    ...
    fn reference_snapshot_boundary_epoch_coverage_summary_reports_the_sparse_epochs() {
        ...
        assert!(
            summary
                .summary_line()
                .contains("Reference snapshot boundary epoch coverage: 90 exact samples across 6 epochs (JD 2451913.5 (TDB)..JD 2451917.5 (TDB)); epochs:")
        );
        assert!(summary
            .summary_line()
    ...
    fn production_generation_snapshot_window_summary_reports_the_source_windows() {
        ...
        assert_eq!(summary.sample_count, 250);
        assert_eq!(summary.sample_bodies.len(), 15);
        assert_eq!(summary.windows.len(), summary.sample_bodies.len());
        assert_eq!(summary.sample_bodies, reference_bodies());
        assert_eq!(summary.epoch_count, 19);
        ...
        assert!(summary.summary_line().starts_with(
            "Production generation source windows: 250 source-backed samples across 15 bodies and 19 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); windows: "
        ));
    ...
    fn reference_snapshot_summary_validation_rejects_asteroid_row_count_drift() {
        ...
        assert!(matches!(
            ...
            Err(
                ReferenceSnapshotSummaryValidationError::AsteroidRowCountMismatch {
                    asteroid_row_count: 76,
                    derived_asteroid_row_count: 75,
                }
            )
        ));
    }
    ...
    fn reference_snapshot_equatorial_parity_summary_reports_the_expected_coverage() {
        ...
        assert_eq!(
            summary.summary_line(),
            format!(
                "JPL reference snapshot equatorial parity: 250 rows across 15 bodies and 19 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies: {}; mean-obliquity transform against the checked-in ecliptic fixture",
                format_bodies(reference_bodies())
            )
        );
        assert_eq!(summary.validate(), Ok(()));
    ...
    fn reference_snapshot_batch_parity_summary_reports_the_expected_coverage() {
        let summary = reference_snapshot_batch_parity_summary()
            .expect("reference snapshot batch parity summary should exist");
        assert_eq!(summary.snapshot.row_count, 250);
        ...
        assert_eq!(summary.ecliptic_request_count, 125);
        assert_eq!(summary.equatorial_request_count, 125);
        assert_eq!(summary.exact_count, 250);
        ...
        assert_eq!(
            summary.summary_line(),
            format!(
                "JPL reference snapshot batch parity: 250 rows across 15 bodies and 19 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies: {}; frame mix: 125 ecliptic, 125 equatorial; quality counts: Exact=250, Interpolated=0, Approximate=0, Unknown=0; batch/single parity preserved",
                format_bodies(reference_bodies())
            )
        );
        assert_eq!(summary.to_string(), summary.summary_line());
        ...
        assert!(jpl_snapshot_evidence_summary_for_report().contains(
            "JPL reference snapshot batch parity: 250 rows across 15 bodies and 19 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies:"
        ));
    ...
    fn comparison_snapshot_summary_reports_the_expected_coverage() {
        ...
        assert_eq!(summary.row_count, 145);
        assert_eq!(summary.body_count, 10);
        ...
        assert_eq!(
            summary.summary_line(),
            "Comparison snapshot coverage: 145 rows across 10 bodies and 16 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies: Sun, Moon, Mercury, Venus, Mars, Jupiter, Saturn, Uranus, Neptune, Pluto"
        );
        assert_eq!(summary.to_string(), summary.summary_line());
        assert_eq!(
            comparison_snapshot_summary_for_report(),
    ...
    fn comparison_snapshot_body_class_coverage_summary_reports_the_expected_windows() {
        ...
        assert!(summary
            .summary_line()
            .starts_with("Comparison snapshot body-class coverage: 145 rows across 10 bodies and 16 epochs; bodies: "));
    ...
    fn comparison_snapshot_batch_parity_summary_reports_the_expected_coverage() {
        ...
        assert_eq!(summary.snapshot.row_count, 145);
        ...
        assert_eq!(summary.ecliptic_request_count, 73);
        assert_eq!(summary.equatorial_request_count, 72);
        assert_eq!(summary.validate(), Ok(()));
        assert_eq!(summary.validated_summary_line(), Ok(summary.summary_line()));
        assert_eq!(
            summary.summary_line(),
            format!(
                "JPL comparison snapshot batch parity: 145 rows across 10 bodies and 16 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies: {}; frame mix: 73 ecliptic, 72 equatorial; quality counts: Exact=145, Interpolated=0, Approximate=0, Unknown=0; batch/single parity preserved",
    ...
    fn reference_asteroid_source_window_summary_reports_the_expanded_coverage() {
        ...
        assert_eq!(summary.windows.len(), summary.sample_bodies.len());
        assert_eq!(summary.sample_count, 75);
    ...
    fn selected_asteroid_source_evidence_summary_reports_the_expanded_coverage() {
        ...
        assert_eq!(
            summary.summary_line(),
            "Selected asteroid source evidence: 75 source-backed samples across 5 bodies and 15 epochs (JD 2378498.5 (TDB)..JD 2634167.0 (TDB)); bodies: Ceres, Pallas, Juno, Vesta, asteroid:433-Eros"
        );
        assert_eq!(
            summary.summary_line(),
    ...
    fn comparison_snapshot_manifest_parses_the_documented_header_comments() {
        ...
        assert_eq!(
            ...
            "Comparison snapshot source windows: 145 source-backed samples across 10 bodies and 16 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); windows: Sun: 15 samples across 15 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); Moon: 15 samples across 15 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); Mercury: 15 samples across 15 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); Venus: 15 samples across 15 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); Mars: 16 samples across 16 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); Jupiter: 14 samples across 14 epochs at JD 2360233.5 (TDB)..JD 2500000.0 (TDB); Saturn: 14 samples across 14 epochs at JD 2360233.5 (TDB)..JD 2500000.0 (TDB); Uranus: 14 samples across 14 epochs at JD 2360233.5 (TDB)..JD 2500000.0 (TDB); Neptune: 14 samples across 14 epochs at JD 2360233.5 (TDB)..JD 2500000.0 (TDB); Pluto: 13 samples across 13 epochs at JD 2378499.0 (TDB)..JD 2500000.0 (TDB)"
        );
        assert_eq!(
    ...
    fn comparison_snapshot_source_window_summary_reports_the_expected_body_windows() {
        ...
        assert_eq!(summary.validate(), Ok(()));
        assert_eq!(summary.validated_summary_line(), Ok(summary.summary_line()));
        assert!(summary.summary_line().contains("Comparison snapshot source windows: 145 source-backed samples across 10 bodies and 16 epochs"));
        assert!(summary.summary_line().contains(
            "Mars: 16 samples across 16 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB)"
        ));
        assert!(summary.summary_line().contains(
            "Pluto: 13 samples across 13 epochs at JD 2378499.0 (TDB)..JD 2500000.0 (TDB)"
        ));
        assert_eq!(
            comparison_snapshot_source_window_summary_for_report(),
            summary.summary_line()
    ...
    fn reference_snapshot_source_summary_reports_the_expected_provenance() {
        ...
        assert_eq!(body_class_summary.major_body_row_count, 175);
        ...
        assert_eq!(body_class_summary.asteroid_row_count, 75);
        ...
        assert!(body_class_summary
            .summary_line()
            .contains("Reference snapshot body-class coverage: major bodies: 175 rows across 10 bodies and 19 epochs; major windows: "));
        assert!(body_class_summary.summary_line().contains(
            "selected asteroids: 75 rows across 5 bodies and 15 epochs; asteroid windows: "
        ));
        ...
        assert_eq!(pluto_window.sample_count, 16);
        assert_eq!(pluto_window.epoch_count, 16);
    ...
    fn reference_snapshot_body_class_coverage_summary_reports_the_expected_body_classes() {
        ...
        assert_eq!(summary.major_body_row_count, 175);
        assert_eq!(summary.major_bodies.len(), 10);
        assert_eq!(
    ...
    fn reference_snapshot_source_window_summary_reports_the_current_boundary_windows() {
        ...
        assert_eq!(
            summary.summary_line(),
            concat!(
                "Reference snapshot source windows: 250 source-backed samples across 15 bodies and 19 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); windows: ",
                "Sun: 18 samples across 18 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); ",
                "Moon: 18 samples across 18 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); ",
                "Mercury: 18 samples across 18 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); ",
                "Venus: 18 samples across 18 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); ",
                "Mars: 19 samples across 19 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); ",
                "Jupiter: 17 samples across 17 epochs at JD 2360233.5 (TDB)..JD 2500000.0 (TDB); ",
                "Saturn: 17 samples across 17 epochs at JD 2360233.5 (TDB)..JD 2500000.0 (TDB); ",
                "Uranus: 17 samples across 17 epochs at JD 2360233.5 (TDB)..JD 2500000.0 (TDB); ",
                "Neptune: 17 samples across 17 epochs at JD 2360233.5 (TDB)..JD 2500000.0 (TDB); ",
                "Pluto: 16 samples across 16 epochs at JD 2378498.5 (TDB)..JD 2500000.0 (TDB); ",
                "Ceres: 15 samples across 15 epochs at JD 2378498.5 (TDB)..JD 2634167.0 (TDB); ",
                "Pallas: 15 samples across 15 epochs at JD 2378498.5 (TDB)..JD 2634167.0 (TDB); ",
                "Juno: 15 samples across 15 epochs at JD 2378498.5 (TDB)..JD 2634167.0 (TDB); ",
                "Vesta: 15 samples across 15 epochs at JD 2378498.5 (TDB)..JD 2634167.0 (TDB); ",
    ...
    fn reference_snapshot_and_holdout_corpora_remain_anchored_to_the_checked_in_csvs() {
        ...
        assert_eq!(reference.row_count, 250);
...
";
    ...
    fn interpolation_quality_summary_reports_the_worst_case_labels() {
        ...
        assert_eq!(summary.sample_count, 135);
        assert_eq!(summary.body_count, 10);
        assert_eq!(summary.epoch_count, 15);
        assert!(summary.earliest_epoch.julian_day.days() <= summary.latest_epoch.julian_day.days());
        assert_eq!(
        ...
        assert!(rendered.contains("135 samples across 10 bodies and 15 epochs"));
    ...
    fn interpolation_quality_kind_coverage_reports_the_distinct_body_breakdown() {
        ...
        assert_eq!(coverage.sample_count, 135);
        ...
        assert!(rendered.contains("135 samples across 10 bodies ["));
