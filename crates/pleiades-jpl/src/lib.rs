...
const REFERENCE_HIGH_CURVATURE_EPOCHS: [f64; 3] = [2_451_911.5, 2_451_912.5, 2_451_913.5];
...
const REFERENCE_SNAPSHOT_COVERAGE_FALLBACK: &str =
    "major bodies sampled at 1749-12-31 for Sun through Neptune, inner planets sampled across 1800-2500, with an additional 2406 Mars hold-out; outer planets and Pluto sampled at 2400000, 2451545, and 2500000; major bodies sampled at 2001-01-02 through 2001-01-04 for additional boundary coverage; selected asteroids sampled at J2000, 2001-01-01, 2132-08-31, and 2500-01-01.";
...
mod tests {
    ...
    fn reference_snapshot_covers_the_expected_bodies_and_epochs() {
        ...
        assert_eq!(reference_epochs().len(), 11);
        assert_eq!(
            reference_snapshot()
                .iter()
    ...
    fn reference_snapshot_summary_reports_the_expected_coverage() {
        ...
        assert_eq!(
            summary.summary_line(),
            format!(
                "Reference snapshot coverage: 100 rows across 15 bodies and 11 epochs (20 asteroid rows; JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies: {}",
    ...
    fn reference_snapshot_high_curvature_summary_reports_the_expected_window() {
        ...
        assert_eq!(summary.earliest_epoch.julian_day.days(), 2_451_911.5);
        assert_eq!(summary.latest_epoch.julian_day.days(), 2_451_913.5);
        ...
        assert_eq!(
            summary.summary_line(),
            "Reference major-body high-curvature evidence: 30 exact samples across 10 bodies and 3 epochs (JD 2451911.5 (TDB)..JD 2451913.5 (TDB)); bodies: Sun, Moon, Mercury, Venus, Saturn, Uranus, Neptune, Pluto, Mars, Jupiter; high-curvature interpolation window"
        );
        assert_eq!(summary.to_string(), summary.summary_line());
        assert_eq!(
    ...
    fn reference_snapshot_high_curvature_window_summary_reports_the_expected_windows() {
        ...
        assert_eq!(summary.windows[0].sample_count, 3);
        assert_eq!(summary.windows[0].epoch_count, 3);
        assert_eq!(
        ...
        assert_eq!(
            summary.summary_line(),
            "Reference major-body high-curvature windows: 30 source-backed samples across 10 bodies and 3 epochs (JD 2451911.5 (TDB)..JD 2451913.5 (TDB)); windows: Sun: 3 samples across 3 epochs at JD 2451911.5 (TDB)..JD 2451913.5 (TDB); Moon: 3 samples across 3 epochs at JD 2451911.5 (TDB)..JD 2451913.5 (TDB); Mercury: 3 samples across 3 epochs at JD 2451911.5 (TDB)..JD 2451913.5 (TDB); Venus: 3 samples across 3 epochs at JD 2451911.5 (TDB)..JD 2451913.5 (TDB); Saturn: 3 samples across 3 epochs at JD 2451911.5 (TDB)..JD 2451913.5 (TDB); Uranus: 3 samples across 3 epochs at JD 2451911.5 (TDB)..JD 2451913.5 (TDB); Neptune: 3 samples across 3 epochs at JD 2451911.5 (TDB)..JD 2451913.5 (TDB); Pluto: 3 samples across 3 epochs at JD 2451911.5 (TDB)..JD 2451913.5 (TDB); Mars: 3 samples across 3 epochs at JD 2451911.5 (TDB)..JD 2451913.5 (TDB); Jupiter: 3 samples across 3 epochs at JD 2451911.5 (TDB)..JD 2451913.5 (TDB)"
        );
        assert_eq!(summary.to_string(), summary.summary_line());
    ...
    fn production_generation_snapshot_summary_reports_the_boundary_overlay() {
        ...
        assert_eq!(summary.row_count, 102);
        assert_eq!(summary.body_count, 15);
        assert_eq!(summary.bodies, reference_bodies());
        assert_eq!(summary.epoch_count, 11);
    ...
    fn production_generation_snapshot_window_summary_reports_the_source_windows() {
        ...
        assert!(summary.summary_line().starts_with(
            "Production generation source windows: 102 source-backed samples across 15 bodies and 11 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); windows: "
        ));
    ...
    fn reference_snapshot_summary_validation_rejects_asteroid_row_count_drift() {
        ...
        assert!(matches!(
            ...
            Err(
                ReferenceSnapshotSummaryValidationError::AsteroidRowCountMismatch {
                    asteroid_row_count: 21,
                    derived_asteroid_row_count: 20,
                }
            )
        ));
    }
    ...
    fn reference_snapshot_equatorial_parity_summary_reports_the_expected_coverage() {
        ...
        assert_eq!(
            summary.summary_line(),
            format!(
                "JPL reference snapshot equatorial parity: 100 rows across 15 bodies and 11 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies: {}; mean-obliquity transform against the checked-in ecliptic fixture",
                format_bodies(reference_bodies())
            )
        );
        assert_eq!(summary.validate(), Ok(()));
    ...
    fn reference_snapshot_batch_parity_summary_reports_the_expected_coverage() {
        ...
        assert_eq!(summary.exact_count, 100);
        ...
        assert_eq!(
            summary.summary_line(),
            format!(
                "JPL reference snapshot batch parity: 100 rows across 15 bodies and 11 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies: {}; frame mix: 50 ecliptic, 50 equatorial; quality counts: Exact=100, Interpolated=0, Approximate=0, Unknown=0; batch/single parity preserved",
                format_bodies(reference_bodies())
            )
        );
        assert_eq!(summary.to_string(), summary.summary_line());
        ...
        assert!(jpl_snapshot_evidence_summary_for_report().contains(
            "JPL reference snapshot batch parity: 100 rows across 15 bodies and 11 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies:"
        ));
    ...
    fn comparison_snapshot_batch_parity_summary_reports_the_expected_coverage() {
        ...
        assert_eq!(summary.snapshot.row_count, 70);
        assert_eq!(summary.snapshot.body_count, 10);
        assert_eq!(summary.snapshot.epoch_count, 9);
        assert_eq!(
            summary.snapshot.earliest_epoch.julian_day.days(),
            2_360_233.5
        );
        ...
        assert_eq!(summary.ecliptic_request_count, 35);
        assert_eq!(summary.equatorial_request_count, 35);
        assert_eq!(summary.validate(), Ok(()));
        assert_eq!(summary.validated_summary_line(), Ok(summary.summary_line()));
        assert_eq!(
            summary.summary_line(),
            format!(
                "JPL comparison snapshot batch parity: 70 rows across 10 bodies and 9 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies: {}; frame mix: 35 ecliptic, 35 equatorial; quality counts: Exact=70, Interpolated=0, Approximate=0, Unknown=0; batch/single parity preserved",
    ...
    fn comparison_snapshot_manifest_parses_the_documented_header_comments() {
        ...
        assert_eq!(
            ...
            "Comparison snapshot source windows: 70 source-backed samples across 10 bodies and 9 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); windows: Sun: 8 samples across 8 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); Moon: 8 samples across 8 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); Mercury: 8 samples across 8 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); Venus: 8 samples across 8 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); Mars: 9 samples across 9 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); Jupiter: 6 samples across 6 epochs at JD 2360233.5 (TDB)..JD 2500000.0 (TDB); Saturn: 6 samples across 6 epochs at JD 2360233.5 (TDB)..JD 2500000.0 (TDB); Uranus: 6 samples across 6 epochs at JD 2360233.5 (TDB)..JD 2500000.0 (TDB); Neptune: 6 samples across 6 epochs at JD 2360233.5 (TDB)..JD 2500000.0 (TDB); Pluto: 5 samples across 5 epochs at JD 2400000.0 (TDB)..JD 2500000.0 (TDB)"
        );
        assert_eq!(
    ...
    fn comparison_snapshot_source_window_summary_reports_the_expected_body_windows() {
        ...
        assert_eq!(summary.validate(), Ok(()));
        assert_eq!(summary.validated_summary_line(), Ok(summary.summary_line()));
        assert!(summary.summary_line().contains("Comparison snapshot source windows: 70 source-backed samples across 10 bodies and 9 epochs"));
        assert!(summary
            .summary_line()
            .contains("Mars: 9 samples across 9 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB)"));
    ...
    fn reference_snapshot_source_summary_reports_the_expected_provenance() {
        ...
        assert_eq!(body_class_summary.major_body_row_count, 80);
        assert_eq!(body_class_summary.major_bodies.len(), 10);
        assert_eq!(body_class_summary.major_epoch_count, 10);
        ...
        assert!(body_class_summary
            .summary_line()
            .contains("Reference snapshot body-class coverage: major bodies: 80 rows across 10 bodies and 10 epochs; major windows: "));
    ...
    fn reference_snapshot_body_class_coverage_summary_reports_the_expected_body_classes() {
        ...
        assert_eq!(summary.major_body_row_count, 80);
    ...
    fn reference_snapshot_manifest_parses_the_documented_header_comments() {
        ...
        assert_eq!(
            ...
            "Reference snapshot manifest: JPL Horizons reference snapshot.; source=NASA/JPL Horizons API, DE441, geocentric ecliptic J2000 vector tables.; coverage=major bodies sampled at 1749-12-31 for Sun through Neptune, inner planets sampled across 1800-2500, with an additional 2406 Mars hold-out; outer planets and Pluto sampled at 2400000, 2451545, and 2500000; major bodies sampled at 2001-01-02 through 2001-01-04 for additional boundary coverage; selected asteroids sampled at J2000, 2001-01-01, 2132-08-31, and 2500-01-01.; columns=epoch_jd, body, x_km, y_km, z_km"
        );
    }
    ...
    fn interpolation_quality_summary_reports_the_worst_case_labels() {
        ...
        assert!(rendered.contains("60 samples across 10 bodies and 8 epochs"));
    ...
    fn interpolation_quality_kind_coverage_reports_the_distinct_body_breakdown() {
        ...
        assert_eq!(coverage.sample_count, 60);
        ...
        assert!(rendered.contains("60 samples across 10 bodies ["));
