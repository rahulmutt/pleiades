...
const REFERENCE_SNAPSHOT_COVERAGE_FALLBACK: &str =
    "major bodies sampled at 1749-12-31 for Sun through Neptune, inner planets sampled across 1800-2500, with an additional 2406 Mars hold-out; outer planets and Pluto sampled at 2400000, 2451545, and 2500000; major bodies sampled at 2001-01-02 through 2001-01-05 for additional boundary coverage; selected asteroids sampled at J2000, 2001-01-01, 2001-01-05, 2132-08-31, and 2500-01-01.";
...
mod tests {
    ...
    fn reference_snapshot_covers_the_expected_bodies_and_epochs() {
        ...
        assert_eq!(reference_epochs().len(), 12);
        assert_eq!(
            reference_snapshot()
                .iter()
    ...
    fn production_generation_snapshot_summary_reports_the_boundary_overlay() {
        ...
        assert_eq!(summary.row_count, 117);
        assert_eq!(summary.body_count, 15);
        assert_eq!(summary.bodies, reference_bodies());
        assert_eq!(summary.epoch_count, 12);
    ...
    fn production_generation_snapshot_window_summary_reports_the_source_windows() {
        ...
        assert!(summary.summary_line().starts_with(
            "Production generation source windows: 117 source-backed samples across 15 bodies and 12 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); windows: "
        ));
    ...
    fn reference_snapshot_summary_validation_rejects_asteroid_row_count_drift() {
        ...
        assert!(matches!(
            ...
            Err(
                ReferenceSnapshotSummaryValidationError::AsteroidRowCountMismatch {
                    asteroid_row_count: 26,
                    derived_asteroid_row_count: 25,
                }
            )
        ));
    }
    ...
    fn reference_snapshot_equatorial_parity_summary_reports_the_expected_coverage() {
        ...
        assert_eq!(
            summary.summary_line(),
            format!(
                "JPL reference snapshot equatorial parity: 115 rows across 15 bodies and 12 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies: {}; mean-obliquity transform against the checked-in ecliptic fixture",
                format_bodies(reference_bodies())
            )
        );
        assert_eq!(summary.validate(), Ok(()));
    ...
    fn reference_snapshot_batch_parity_summary_reports_the_expected_coverage() {
        let summary = reference_snapshot_batch_parity_summary()
            .expect("reference snapshot batch parity summary should exist");
        assert_eq!(summary.snapshot.row_count, 115);
        ...
        assert_eq!(summary.ecliptic_request_count, 58);
        assert_eq!(summary.equatorial_request_count, 57);
        assert_eq!(summary.exact_count, 115);
        ...
        assert_eq!(
            summary.summary_line(),
            format!(
                "JPL reference snapshot batch parity: 115 rows across 15 bodies and 12 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies: {}; frame mix: 58 ecliptic, 57 equatorial; quality counts: Exact=115, Interpolated=0, Approximate=0, Unknown=0; batch/single parity preserved",
                format_bodies(reference_bodies())
            )
        );
        assert_eq!(summary.to_string(), summary.summary_line());
        ...
        assert!(jpl_snapshot_evidence_summary_for_report().contains(
            "JPL reference snapshot batch parity: 115 rows across 15 bodies and 12 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies:"
        ));
    ...
    fn comparison_snapshot_summary_reports_the_expected_coverage() {
        ...
        assert_eq!(
            summary.summary_line(),
            "Comparison snapshot coverage: 80 rows across 10 bodies and 10 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies: Sun, Moon, Mercury, Venus, Mars, Jupiter, Saturn, Uranus, Neptune, Pluto"
        );
        assert_eq!(summary.to_string(), summary.summary_line());
        assert_eq!(
            comparison_snapshot_summary_for_report(),
    ...
    fn comparison_snapshot_body_class_coverage_summary_reports_the_expected_windows() {
        ...
        assert!(summary
            .summary_line()
            .starts_with("Comparison snapshot body-class coverage: 80 rows across 10 bodies and 10 epochs; bodies: "));
    ...
    fn comparison_snapshot_batch_parity_summary_reports_the_expected_coverage() {
        ...
        assert_eq!(summary.snapshot.row_count, 80);
        assert_eq!(summary.snapshot.body_count, 10);
        assert_eq!(summary.snapshot.epoch_count, 10);
        assert_eq!(
            summary.snapshot.earliest_epoch.julian_day.days(),
            2_360_233.5
        );
        ...
        assert_eq!(summary.ecliptic_request_count, 40);
        assert_eq!(summary.equatorial_request_count, 40);
        assert_eq!(summary.validate(), Ok(()));
        assert_eq!(summary.validated_summary_line(), Ok(summary.summary_line()));
        assert_eq!(
            summary.summary_line(),
            format!(
                "JPL comparison snapshot batch parity: 80 rows across 10 bodies and 10 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies: {}; frame mix: 40 ecliptic, 40 equatorial; quality counts: Exact=80, Interpolated=0, Approximate=0, Unknown=0; batch/single parity preserved",
    ...
    fn selected_asteroid_source_evidence_summary_reports_the_expanded_coverage() {
        ...
        assert_eq!(
            summary.summary_line(),
            "Selected asteroid source evidence: 25 source-backed samples across 5 bodies and 5 epochs (JD 2451545.0 (TDB)..JD 2634167.0 (TDB)); bodies: Ceres, Pallas, Juno, Vesta, asteroid:433-Eros"
        );
        assert_eq!(
            summary.summary_line(),
    ...
    fn comparison_snapshot_manifest_parses_the_documented_header_comments() {
        ...
        assert_eq!(
            ...
            "Comparison snapshot source windows: 80 source-backed samples across 10 bodies and 10 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); windows: Sun: 9 samples across 9 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); Moon: 9 samples across 9 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); Mercury: 9 samples across 9 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); Venus: 9 samples across 9 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); Mars: 10 samples across 10 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); Jupiter: 7 samples across 7 epochs at JD 2360233.5 (TDB)..JD 2500000.0 (TDB); Saturn: 7 samples across 7 epochs at JD 2360233.5 (TDB)..JD 2500000.0 (TDB); Uranus: 7 samples across 7 epochs at JD 2360233.5 (TDB)..JD 2500000.0 (TDB); Neptune: 7 samples across 7 epochs at JD 2360233.5 (TDB)..JD 2500000.0 (TDB); Pluto: 6 samples across 6 epochs at JD 2400000.0 (TDB)..JD 2500000.0 (TDB)"
        );
        assert_eq!(
    ...
    fn comparison_snapshot_source_window_summary_reports_the_expected_body_windows() {
        ...
        assert_eq!(summary.validate(), Ok(()));
        assert_eq!(summary.validated_summary_line(), Ok(summary.summary_line()));
        assert!(summary.summary_line().contains("Comparison snapshot source windows: 80 source-backed samples across 10 bodies and 10 epochs"));
        assert!(summary.summary_line().contains(
            "Mars: 10 samples across 10 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB)"
        ));
        assert!(summary.summary_line().contains(
            "Pluto: 6 samples across 6 epochs at JD 2400000.0 (TDB)..JD 2500000.0 (TDB)"
        ));
        assert_eq!(
            comparison_snapshot_source_window_summary_for_report(),
            summary.summary_line()
    ...
    fn reference_snapshot_source_summary_reports_the_expected_provenance() {
        ...
        assert_eq!(body_class_summary.major_body_row_count, 90);
        assert_eq!(body_class_summary.major_bodies.len(), 10);
        assert_eq!(body_class_summary.major_epoch_count, 11);
        assert_eq!(body_class_summary.major_windows.len(), 10);
        assert_eq!(body_class_summary.asteroid_row_count, 25);
        assert_eq!(body_class_summary.asteroid_bodies.len(), 5);
        assert_eq!(body_class_summary.asteroid_epoch_count, 5);
        ...
        assert!(body_class_summary
            .summary_line()
            .contains("Reference snapshot body-class coverage: major bodies: 90 rows across 10 bodies and 11 epochs; major windows: "));
        assert!(body_class_summary.summary_line().contains(
            "selected asteroids: 25 rows across 5 bodies and 5 epochs; asteroid windows: "
        ));
    ...
    fn reference_snapshot_body_class_coverage_summary_reports_the_expected_body_classes() {
        ...
        assert_eq!(summary.major_body_row_count, 90);
        ...
        assert_eq!(summary.asteroid_epoch_count, 5);
        assert_eq!(summary.validate(), Ok(()));
        assert_eq!(summary.validated_summary_line(), Ok(summary.summary_line()));
        assert_eq!(
            reference_snapshot_body_class_coverage_summary_for_report(),
    ...
    fn reference_snapshot_manifest_parses_the_documented_header_comments() {
        ...
        assert_eq!(
            ...
            "Reference snapshot manifest: JPL Horizons reference snapshot.; source=NASA/JPL Horizons API, DE441, geocentric ecliptic J2000 vector tables.; coverage=major bodies sampled at 1749-12-31 for Sun through Neptune, inner planets sampled across 1800-2500, with an additional 2406 Mars hold-out; outer planets and Pluto sampled at 2400000, 2451545, and 2500000; major bodies sampled at 2001-01-02 through 2001-01-05 for additional boundary coverage; selected asteroids sampled at J2000, 2001-01-01, 2001-01-05, 2132-08-31, and 2500-01-01.; columns=epoch_jd, body, x_km, y_km, z_km"
        );
    }
    ...
    fn interpolation_quality_summary_reports_the_worst_case_labels() {
        ...
        assert_eq!(summary.sample_count, 70);
        assert_eq!(summary.body_count, 10);
        ...
        assert!(rendered.contains("70 samples across 10 bodies and 9 epochs"));
    ...
    fn interpolation_quality_kind_coverage_reports_the_distinct_body_breakdown() {
        ...
        assert_eq!(coverage.sample_count, 70);
        ...
        assert!(rendered.contains("70 samples across 10 bodies ["));
