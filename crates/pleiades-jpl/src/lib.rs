...
mod tests {
    ...
    fn independent_holdout_snapshot_summary_reports_the_expected_coverage() {
        ...
        assert_eq!(summary.row_count, 33);
        assert_eq!(summary.body_count, 10);
        assert_eq!(
            summary.bodies,
            vec![
        ...
        assert_eq!(
            summary.summary_line(),
            "Independent hold-out coverage: 33 rows across 10 bodies and 8 epochs (JD 2400000.0 (TDB)..JD 2634167.0 (TDB)); bodies: Mars, Jupiter, Mercury, Venus, Saturn, Uranus, Neptune, Sun, Moon, Pluto"
        );
        assert_eq!(summary.to_string(), summary.summary_line());
        assert_eq!(summary.validated_summary_line(), Ok(summary.summary_line()));
        assert_eq!(
    ...
    fn independent_holdout_snapshot_equatorial_parity_summary_reports_the_expected_coverage() {
        ...
        assert_eq!(
            summary.summary_line(),
            "JPL independent hold-out equatorial parity: 33 rows across 10 bodies and 8 epochs (JD 2400000.0 (TDB)..JD 2634167.0 (TDB)); mean-obliquity transform against the checked-in ecliptic fixture"
        );
        assert_eq!(summary.validate(), Ok(()));
        assert_eq!(summary.validated_summary_line(), Ok(summary.summary_line()));
        assert_eq!(
    ...
    fn independent_holdout_summary_reports_the_expected_envelope() {
        ...
        assert!(rendered.contains(
            "33 exact rows across 10 bodies (Mars, Jupiter, Mercury, Venus, Saturn, Uranus, Neptune, Sun, Moon, Pluto) and 8 epochs"
        ));
    ...
    fn batch_query_preserves_independent_holdout_mixed_scale_order_and_single_query_parity() {
        ...
        assert_eq!(summary.to_string(), summary.summary_line());
        assert_eq!(summary.snapshot.row_count, 33);
        assert_eq!(summary.snapshot.body_count, 10);
        assert_eq!(summary.tt_request_count, 17);
        assert_eq!(summary.tdb_request_count, 16);
        ...
        assert!(rendered.contains(
            "33 requests across 10 bodies (Mars, Jupiter, Mercury, Venus, Saturn, Uranus, Neptune, Sun, Moon, Pluto) and 8 epochs"
        ));
        assert!(rendered.contains("TT requests=17, TDB requests=16"));
