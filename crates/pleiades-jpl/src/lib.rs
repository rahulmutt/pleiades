...
const REFERENCE_SNAPSHOT_COVERAGE_FALLBACK: &str =
    "major bodies sampled at 1749-12-31 for Sun through Neptune, inner planets sampled across 1800-2500; major bodies sampled at 1800-01-03 for Sun through Pluto; major bodies sampled at 2400000, 2451545, 2451910.5, 2451911.5, 2451912.5, 2451913.5, 2451914.0, 2451914.5, 2451915.0, 2451915.5, 2451916.5, 2451917.5, 2451918.5, 2453000.5, and 2500000; major bodies sampled at 2451915.5 for Sun through Pluto; Mars sampled at 2600000 and 2634167 for outer boundary coverage; major bodies sampled at 2451913.5 through 2451917.5 for additional boundary coverage; selected asteroids sampled at J2000, 2378498.5, 2451910.5 through 2451918.5, with 2451914.0, 2451914.5, 2451915.0, 2451915.5, and 2451918.5 boundary coverage, 1800-01-03, 2003-12-27, 2132-08-31, 2500-01-01, and 2634167.";
...
mod tests {
    ...
    fn reference_snapshot_covers_the_expected_bodies_and_epochs() {
        ...
        assert_eq!(reference_epochs().len(), 20);
        assert_eq!(
            reference_snapshot()
                .iter()
    ...
    fn reference_snapshot_summary_reports_the_expected_coverage() {
        ...
        assert_eq!(summary.row_count, 265);
        assert_eq!(summary.body_count, 15);
        assert_eq!(summary.bodies, reference_bodies());
        assert_eq!(summary.epoch_count, 20);
        assert_eq!(summary.asteroid_row_count, 80);
        assert_eq!(summary.earliest_epoch.julian_day.days(), 2_360_233.5);
        assert_eq!(summary.latest_epoch.julian_day.days(), 2_634_167.0);
        assert_eq!(
            summary.summary_line(),
            format!(
                "Reference snapshot coverage: 265 rows across 15 bodies and 20 epochs (80 asteroid rows; JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies: {}",
    ...
    fn reference_snapshot_boundary_epoch_coverage_summary_reports_the_sparse_epochs() {
        ...
        assert_eq!(summary.sample_count, 105);
        ...
        assert_eq!(summary.windows.len(), 7);
        ...
        assert!(
            summary
                .summary_line()
                .contains("Reference snapshot boundary epoch coverage: 105 exact samples across 7 epochs (JD 2451913.5 (TDB)..JD 2451917.5 (TDB)); epochs:")
        );
    ...
    fn reference_snapshot_sparse_boundary_summary_reports_the_asteroid_only_day() {
        ...
        assert_eq!(
            summary.summary_line(),
            "Reference snapshot boundary day: 15 exact samples at JD 2451915.5 (TDB) (Sun, Moon, Mercury, Venus, Mars, Jupiter, Saturn, Uranus, Neptune, Pluto, Ceres, Pallas, Juno, Vesta, asteroid:433-Eros)"
        );
        assert_eq!(summary.to_string(), summary.summary_line());
        assert_eq!(
            reference_snapshot_sparse_boundary_summary_for_report(),
    ...
    fn reference_snapshot_mars_jupiter_boundary_summary_reports_the_boundary_day() {
        ...
        assert_eq!(
            summary.summary_line(),
            "Reference Mars/Jupiter boundary evidence: 15 exact samples at JD 2451918.5 (TDB) (Sun, Moon, Mercury, Venus, Mars, Jupiter, Saturn, Uranus, Neptune, Pluto, Ceres, Pallas, Juno, Vesta, asteroid:433-Eros); 2001-01-09 boundary sample"
        );
        assert_eq!(summary.to_string(), summary.summary_line());
        assert_eq!(
    ...
    fn production_generation_snapshot_window_summary_reports_the_source_windows() {
        ...
        assert_eq!(summary.sample_count, 265);
        assert_eq!(summary.sample_bodies.len(), 15);
        assert_eq!(summary.windows.len(), summary.sample_bodies.len());
        assert_eq!(summary.sample_bodies, reference_bodies());
        assert_eq!(summary.epoch_count, 20);
        ...
        assert!(summary.summary_line().starts_with(
            "Production generation source windows: 265 source-backed samples across 15 bodies and 20 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); windows: "
        ));
    ...
    fn reference_snapshot_summary_validation_rejects_asteroid_row_count_drift() {
        ...
        assert!(matches!(
            ...
            Err(
                ReferenceSnapshotSummaryValidationError::AsteroidRowCountMismatch {
                    asteroid_row_count: 81,
                    derived_asteroid_row_count: 80,
                }
            )
        ));
    }
    ...
    fn reference_snapshot_equatorial_parity_summary_reports_the_expected_coverage() {
        ...
        assert_eq!(
            summary.summary_line(),
            format!(
                "JPL reference snapshot equatorial parity: 265 rows across 15 bodies and 20 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies: {}; mean-obliquity transform against the checked-in ecliptic fixture",
                format_bodies(reference_bodies())
            )
        );
        assert_eq!(summary.validate(), Ok(()));
    ...
    fn reference_snapshot_batch_parity_summary_reports_the_expected_coverage() {
        ...
        assert_eq!(summary.exact_count, 265);
        ...
        assert!(summary
            .summary_line()
            .contains("JPL reference snapshot batch parity: 265 rows across 15 bodies and 20 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies: "));
        assert!(summary
            .summary_line()
            .contains("quality counts: Exact=265, Interpolated=0, Approximate=0, Unknown=0; batch/single parity preserved"));
        assert_eq!(summary.to_string(), summary.summary_line());
        assert_eq!(summary.validated_summary_line(), Ok(summary.summary_line()));
        assert_eq!(
            reference_snapshot_batch_parity_summary_for_report(),
        ...
        assert!(jpl_snapshot_evidence_summary_for_report().contains(
            "JPL reference snapshot batch parity: 265 rows across 15 bodies and 20 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies:"
        ));
    ...
    fn comparison_snapshot_summary_reports_the_expected_coverage() {
        ...
        assert_eq!(summary.row_count, 155);
        assert_eq!(summary.body_count, 10);
        ...
        assert_eq!(
            summary.summary_line(),
            "Comparison snapshot coverage: 155 rows across 10 bodies and 17 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies: Sun, Moon, Mercury, Venus, Mars, Jupiter, Saturn, Uranus, Neptune, Pluto"
        );
        assert_eq!(summary.to_string(), summary.summary_line());
        assert_eq!(
            comparison_snapshot_summary_for_report(),
    ...
    fn comparison_snapshot_body_class_coverage_summary_reports_the_expected_windows() {
        ...
        assert!(summary
            .summary_line()
            .starts_with("Comparison snapshot body-class coverage: 155 rows across 10 bodies and 17 epochs; bodies: "));
    ...
    fn comparison_snapshot_batch_parity_summary_reports_the_expected_coverage() {
        ...
        assert_eq!(summary.snapshot.row_count, 155);
        ...
        assert_eq!(summary.ecliptic_request_count, 78);
        assert_eq!(summary.equatorial_request_count, 77);
        assert_eq!(summary.validate(), Ok(()));
        assert_eq!(summary.validated_summary_line(), Ok(summary.summary_line()));
        assert_eq!(
            summary.summary_line(),
            format!(
                "JPL comparison snapshot batch parity: 155 rows across 10 bodies and 17 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies: {}; frame mix: 78 ecliptic, 77 equatorial; quality counts: Exact=155, Interpolated=0, Approximate=0, Unknown=0; batch/single parity preserved",
    ...
    fn selected_asteroid_source_evidence_summary_reports_the_expanded_coverage() {
        ...
        assert_eq!(
            summary.summary_line(),
            "Selected asteroid source evidence: 80 source-backed samples across 5 bodies and 16 epochs (JD 2378498.5 (TDB)..JD 2634167.0 (TDB)); bodies: Ceres, Pallas, Juno, Vesta, asteroid:433-Eros"
        );
        assert_eq!(
            summary.summary_line(),
    ...
    fn comparison_snapshot_manifest_parses_the_documented_header_comments() {
        ...
        assert_eq!(
            ...
            "Comparison snapshot source windows: 155 source-backed samples across 10 bodies and 17 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); windows: Sun: 16 samples across 16 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); Moon: 16 samples across 16 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); Mercury: 16 samples across 16 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); Venus: 16 samples across 16 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); Mars: 17 samples across 17 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); Jupiter: 15 samples across 15 epochs at JD 2360233.5 (TDB)..JD 2500000.0 (TDB); Saturn: 15 samples across 15 epochs at JD 2360233.5 (TDB)..JD 2500000.0 (TDB); Uranus: 15 samples across 15 epochs at JD 2360233.5 (TDB)..JD 2500000.0 (TDB); Neptune: 15 samples across 15 epochs at JD 2360233.5 (TDB)..JD 2500000.0 (TDB); Pluto: 14 samples across 14 epochs at JD 2378499.0 (TDB)..JD 2500000.0 (TDB)"
        );
        assert_eq!(
    ...
    fn comparison_snapshot_source_window_summary_reports_the_expected_body_windows() {
        let summary = comparison_snapshot_source_window_summary()
            .expect("comparison snapshot source window summary should exist");
        assert_eq!(summary.sample_count, 155);
        assert_eq!(summary.sample_bodies.len(), 10);
        assert_eq!(summary.epoch_count, 17);
        assert_eq!(summary.windows.len(), 10);
        assert_eq!(summary.validate(), Ok(()));
        assert_eq!(summary.validated_summary_line(), Ok(summary.summary_line()));
        assert!(summary.summary_line().contains("Comparison snapshot source windows: 155 source-backed samples across 10 bodies and 17 epochs"));
        assert!(summary.summary_line().contains(
            "Mars: 17 samples across 17 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB)"
        ));
        assert!(summary.summary_line().contains(
            "Pluto: 14 samples across 14 epochs at JD 2378499.0 (TDB)..JD 2500000.0 (TDB)"
        ));
        assert_eq!(
            comparison_snapshot_source_window_summary_for_report(),
            summary.summary_line()
    ...
    fn reference_snapshot_source_summary_reports_the_expected_provenance() {
        ...
        assert_eq!(body_class_summary.major_body_row_count, 185);
        ...
        assert_eq!(body_class_summary.asteroid_row_count, 80);
        assert_eq!(body_class_summary.asteroid_bodies.len(), 5);
        assert_eq!(body_class_summary.asteroid_epoch_count, 16);
        ...
        assert!(body_class_summary
            .summary_line()
            .contains("Reference snapshot body-class coverage: major bodies: 185 rows across 10 bodies and 20 epochs; major windows: "));
        assert!(body_class_summary.summary_line().contains(
            "selected asteroids: 80 rows across 5 bodies and 16 epochs; asteroid windows: "
        ));
        ...
        assert_eq!(sun_window.sample_count, 19);
        assert_eq!(sun_window.epoch_count, 19);
        ...
        assert_eq!(jupiter_window.sample_count, 18);
        assert_eq!(jupiter_window.epoch_count, 18);
        ...
        assert_eq!(asteroid_window.sample_count, 16);
        assert_eq!(asteroid_window.epoch_count, 16);
        assert_eq!(
    ...
    fn reference_snapshot_body_class_coverage_summary_reports_the_expected_body_classes() {
        ...
        assert_eq!(summary.major_body_row_count, 185);
        assert_eq!(summary.major_bodies.len(), 10);
        assert_eq!(
        ...
        assert_eq!(summary.asteroid_epoch_count, 16);
        assert_eq!(summary.validate(), Ok(()));
        assert_eq!(summary.validated_summary_line(), Ok(summary.summary_line()));
        assert_eq!(
            reference_snapshot_body_class_coverage_summary_for_report(),
    ...
    fn reference_snapshot_source_window_summary_reports_the_current_boundary_windows() {
        ...
        assert_eq!(
            summary.summary_line(),
            concat!(
                "Reference snapshot source windows: 265 source-backed samples across 15 bodies and 20 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); windows: ",
                "Sun: 19 samples across 19 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); ",
                "Moon: 19 samples across 19 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); ",
                "Mercury: 19 samples across 19 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); ",
                "Venus: 19 samples across 19 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); ",
                "Mars: 20 samples across 20 epochs at JD 2360233.5 (TDB)..JD 2634167.0 (TDB); ",
                "Jupiter: 18 samples across 18 epochs at JD 2360233.5 (TDB)..JD 2500000.0 (TDB); ",
                "Saturn: 18 samples across 18 epochs at JD 2360233.5 (TDB)..JD 2500000.0 (TDB); ",
                "Uranus: 18 samples across 18 epochs at JD 2360233.5 (TDB)..JD 2500000.0 (TDB); ",
                "Neptune: 18 samples across 18 epochs at JD 2360233.5 (TDB)..JD 2500000.0 (TDB); ",
                "Pluto: 17 samples across 17 epochs at JD 2378498.5 (TDB)..JD 2500000.0 (TDB); ",
                "Ceres: 16 samples across 16 epochs at JD 2378498.5 (TDB)..JD 2634167.0 (TDB); ",
                "Pallas: 16 samples across 16 epochs at JD 2378498.5 (TDB)..JD 2634167.0 (TDB); ",
                "Juno: 16 samples across 16 epochs at JD 2378498.5 (TDB)..JD 2634167.0 (TDB); ",
                "Vesta: 16 samples across 16 epochs at JD 2378498.5 (TDB)..JD 2634167.0 (TDB); ",
                "asteroid:433-Eros: 16 samples across 16 epochs at JD 2378498.5 (TDB)..JD 2634167.0 (TDB)"
            )
        );
    }
...
";
    ...
    fn interpolation_quality_summary_reports_the_worst_case_labels() {
        ...
        assert_eq!(summary.sample_count, 145);
        assert_eq!(summary.body_count, 10);
        assert_eq!(summary.epoch_count, 16);
        assert!(summary.earliest_epoch.julian_day.days() <= summary.latest_epoch.julian_day.days());
        assert_eq!(
        ...
        assert!(rendered.contains("145 samples across 10 bodies and 16 epochs"));
    ...
    fn interpolation_quality_kind_coverage_reports_the_distinct_body_breakdown() {
        ...
        assert_eq!(coverage.sample_count, 145);
        ...
        assert!(rendered.contains("145 samples across 10 bodies ["));
