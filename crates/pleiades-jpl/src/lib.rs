...
const INDEPENDENT_HOLDOUT_COVERAGE_FALLBACK: &str =
    "Mars and Jupiter at 2001-01-01 through 2001-01-03, plus Jupiter at 2400000, plus Mercury and Venus at 2451545, 2500000, and 2634167, plus Saturn at 2400000, 2451545, and 2500000, plus Uranus and Neptune at 2451545 and 2500000, plus Mars at 2451545, 2500000, 2600000, and 2634167, plus Sun at 2451545, 2500000, and 2634167, plus Pluto at 2451545.";
...
mod tests {
    ...
    fn independent_holdout_snapshot_summary_reports_the_expected_coverage() {
        ...
        assert_eq!(summary.row_count, 28);
        ...
        assert_eq!(
            summary.summary_line(),
            "Independent hold-out coverage: 28 rows across 9 bodies and 8 epochs (JD 2400000.0 (TDB)..JD 2634167.0 (TDB)); bodies: Mars, Jupiter, Mercury, Venus, Saturn, Uranus, Neptune, Sun, Pluto"
        );
        assert_eq!(summary.to_string(), summary.summary_line());
        assert_eq!(
            independent_holdout_snapshot_summary_for_report(),
    ...
    fn independent_holdout_snapshot_equatorial_parity_summary_reports_the_expected_coverage() {
        ...
        assert_eq!(
            summary.summary_line(),
            "JPL independent hold-out equatorial parity: 28 rows across 9 bodies and 8 epochs (JD 2400000.0 (TDB)..JD 2634167.0 (TDB)); mean-obliquity transform against the checked-in ecliptic fixture"
        );
        assert_eq!(summary.validate(), Ok(()));
        assert_eq!(summary.validated_summary_line(), Ok(summary.summary_line()));
        assert_eq!(
    ...
    fn independent_holdout_summary_reports_the_expected_envelope() {
        ...
        assert_eq!(summary.sample_count, 28);
        assert_eq!(summary.body_count, 9);
        assert_eq!(
            summary.bodies,
            vec![
        ...
        assert!(rendered.contains(
            "28 exact rows across 9 bodies (Mars, Jupiter, Mercury, Venus, Saturn, Uranus, Neptune, Sun, Pluto) and 8 epochs"
        ));
    ...
    fn batch_query_preserves_independent_holdout_mixed_scale_order_and_single_query_parity() {
        ...
        assert_eq!(summary.to_string(), summary.summary_line());
        assert_eq!(summary.snapshot.row_count, 28);
        assert_eq!(summary.snapshot.body_count, 9);
        assert_eq!(summary.tt_request_count, 14);
        assert_eq!(summary.tdb_request_count, 14);
        ...
        assert!(rendered.contains(
            "28 requests across 9 bodies (Mars, Jupiter, Mercury, Venus, Saturn, Uranus, Neptune, Sun, Pluto) and 8 epochs"
        ));
        assert!(rendered.contains("TT requests=14, TDB requests=14"));
