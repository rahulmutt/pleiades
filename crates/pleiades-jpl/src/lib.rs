...
const INDEPENDENT_HOLDOUT_COVERAGE_FALLBACK: &str =
    "Mars and Jupiter at 2001-01-01 through 2001-01-03, plus Saturn at 2400000, 2451545, and 2500000, plus Uranus and Neptune at 2451545 and 2500000, plus Mars at 2451545, 2500000, 2600000, and 2634167.";
...
mod tests {
    ...
    fn independent_holdout_source_summary_reports_the_expected_provenance() {
        ...
        assert_eq!(
            summary.summary_line(),
            "Independent hold-out source: NASA/JPL Horizons API, DE441, geocentric ecliptic J2000 vector tables.; coverage=Mars and Jupiter at 2001-01-01 through 2001-01-03, plus Saturn at 2400000, 2451545, and 2500000, plus Uranus and Neptune at 2451545 and 2500000, plus Mars at 2451545, 2500000, 2600000, and 2634167.; columns=epoch_jd, body, x_km, y_km, z_km"
        );
        assert_eq!(summary.to_string(), summary.summary_line());
        assert_eq!(summary.validate(), Ok(()));
        assert_eq!(
    ...
    fn independent_holdout_snapshot_summary_reports_the_expected_coverage() {
        ...
        assert_eq!(
            summary.summary_line(),
            "Independent hold-out coverage: 17 rows across 5 bodies and 8 epochs (JD 2400000.0 (TDB)..JD 2634167.0 (TDB)); bodies: Mars, Jupiter, Saturn, Uranus, Neptune"
        );
        assert_eq!(summary.to_string(), summary.summary_line());
        assert_eq!(
            independent_holdout_snapshot_summary_for_report(),
    ...
    fn independent_holdout_snapshot_equatorial_parity_summary_reports_the_expected_coverage() {
        ...
        assert_eq!(
            summary.summary_line(),
            "JPL independent hold-out equatorial parity: 17 rows across 5 bodies and 8 epochs (JD 2400000.0 (TDB)..JD 2634167.0 (TDB)); mean-obliquity transform against the checked-in ecliptic fixture"
        );
        assert_eq!(summary.validate(), Ok(()));
        assert_eq!(
            independent_holdout_snapshot_equatorial_parity_summary_for_report(),
    ...
    fn independent_holdout_summary_reports_the_expected_envelope() {
        ...
        assert!(rendered.contains(
            "17 exact rows across 5 bodies (Mars, Jupiter, Saturn, Uranus, Neptune) and 8 epochs"
        ));
    ...
    fn batch_query_preserves_independent_holdout_mixed_scale_order_and_single_query_parity() {
        ...
        assert_eq!(summary.to_string(), summary.summary_line());
        assert_eq!(summary.snapshot.row_count, 17);
        assert_eq!(summary.snapshot.body_count, 5);
        assert_eq!(summary.tt_request_count, 9);
        assert_eq!(summary.tdb_request_count, 8);
        ...
        assert!(rendered.contains(
            "17 requests across 5 bodies (Mars, Jupiter, Saturn, Uranus, Neptune) and 8 epochs"
        ));
        assert!(rendered.contains("TT requests=9, TDB requests=8"));
    ...
    fn independent_holdout_snapshot_manifest_parses_the_documented_header_comments() {
        ...
        assert_eq!(
            ...
            Some("Mars and Jupiter at 2001-01-01 through 2001-01-03, plus Saturn at 2400000, 2451545, and 2500000, plus Uranus and Neptune at 2451545 and 2500000, plus Mars at 2451545, 2500000, 2600000, and 2634167.")
        );
        assert_eq!(
            manifest.columns,
            ["epoch_jd", "body", "x_km", "y_km", "z_km"]
