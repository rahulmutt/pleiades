    ...
    fn comparison_body_class_tolerance_summary_and_alias_commands_render_the_posture() {
        ...
        assert_eq!(
            render_cli(&["comparison-body-class-tolerance-summary", "extra"]).expect_err(
                "comparison body-class tolerance summary should reject extra arguments"
            ),
    ...
    fn comparison_body_class_error_envelope_summary_and_alias_commands_render_the_envelopes() {
        ...
        assert_eq!(
            render_cli(&["comparison-body-class-error-envelope", "extra"]).expect_err(
                "comparison body-class error envelope alias should reject extra arguments"
            ),
    ...
    fn packaged_artifact_fit_threshold_violations_summary_reflects_the_current_posture() {
        ...
        assert!(
            alias_rendered.contains("Packaged-artifact fit threshold violations: 0; details: none")
        );
    }
    ...
    fn validation_report_includes_corpus_metadata() {
        ...
        assert!(report
            .contains("Comparison snapshot coverage: 232 rows across 10 bodies and 28 epochs"));
        assert!(report.contains("Apparentness: Mean"));
    ...
    fn cli_help_lists_the_validation_commands() {
        ...
        assert!(
            rendered.contains("compatibility-caveats    Alias for compatibility-caveats-summary")
        );
        ...
        assert!(rendered
            .contains("lunar-reference-evidence  Alias for lunar-reference-evidence-summary"));
        ...
        assert!(
            rendered.contains("jpl-source-posture         Alias for jpl-source-posture-summary")
        );
        ...
        assert!(rendered
            .contains("packaged-artifact-access  Alias for packaged-artifact-access-summary"));
        assert!(rendered.contains(
        ...
        assert!(rendered
            .contains("packaged-frame-parity         Alias for packaged-frame-parity-summary"));
        ...
        assert!(
            rendered.contains("house-formula-families    Alias for house-formula-families-summary")
        );
        ...
        assert!(rendered
            .contains("house-latitude-sensitive  Alias for house-latitude-sensitive-summary"));
        ...
        assert!(
            rendered.contains("utc-convenience-policy  Alias for utc-convenience-policy-summary")
        );
        ...
        assert!(
            rendered.contains("production-generation-boundary-request-corpus-equatorial-summary")
        );
    ...
    fn regenerate_packaged_artifact_command_writes_all_sidecars() {
        ...
        let manifest_checksum_path =
            output_alias_dir.join("packaged-artifact.manifest.checksum.txt");
    ...
    fn release_notes_summary_command_renders_the_summary() {
        ...
        assert!(
            rendered.contains(&reference_snapshot_2451917_major_body_boundary_summary_for_report())
        );
        assert!(
    ...
    fn release_summary_command_renders_the_quick_overview() {
        ...
        assert!(
            rendered.contains(&reference_snapshot_1750_major_body_interior_summary_for_report())
        );
        assert!(
            rendered.contains(&reference_snapshot_2451916_major_body_interior_summary_for_report())
        );
        assert!(
    ...
    fn backend_matrix_summary_command_renders_the_summary() {
        ...
        assert!(
            rendered.contains(&reference_snapshot_1600_selected_body_boundary_summary_for_report())
        );
        assert!(rendered.contains(&reference_snapshot_high_curvature_summary_for_report()));
        assert!(
            rendered.contains(&reference_snapshot_2500_major_body_boundary_summary_for_report())
    ...
    fn workspace_provenance_summary_reports_workspace_tool_versions() {
        ...
        let alias = render_cli(&["workspace-provenance"])
            .expect("workspace provenance alias should render");
...
"#;
    ...
    fn release_bundle_writes_expected_artifacts() {
        ...
        assert!(
            rendered.contains("reference-snapshot-2451916-major-body-dense-boundary-summary.txt")
        );
        ...
        assert!(
            release_summary.contains(&reference_snapshot_pre_bridge_boundary_summary_for_report())
        );
        ...
        assert!(
            backend_matrix_summary.contains("Selected asteroid evidence: 6 exact J2000 samples")
        );
        ...
        assert!(
            manifest.contains("production generation corpus shape summary checksum (fnv1a-64): 0x")
        );
        ...
        assert!(
            manifest.contains("reference-snapshot-2451916-major-body-dense-boundary-summary.txt")
        );
        assert!(manifest.contains(
        ...
        assert!(manifest.contains(
            "reference snapshot major-body boundary window summary checksum (fnv1a-64): 0x"
        ));
        assert!(manifest.contains(
            "reference snapshot boundary epoch coverage summary checksum (fnv1a-64): 0x"
        ));
        assert!(manifest
            .contains("reference snapshot pre-bridge boundary summary checksum (fnv1a-64): 0x"));
        assert!(manifest.contains(
        ...
        assert!(
            manifest.contains("reference snapshot sparse boundary summary checksum (fnv1a-64): 0x")
        );
        ...
        assert!(
            manifest.contains("reference snapshot source window summary checksum (fnv1a-64): 0x")
        );
        assert!(manifest
            .contains("reference snapshot body-class coverage summary checksum (fnv1a-64): 0x"));
        assert!(manifest
            .contains("reference snapshot equatorial parity summary checksum (fnv1a-64): 0x"));
        ...
        assert!(manifest
            .contains("production generation boundary window summary checksum (fnv1a-64): 0x"));
        assert!(manifest.contains("production-generation-boundary-request-corpus-summary.txt"));
        assert!(manifest
            .contains("production-generation-boundary-request-corpus-equatorial-summary.txt"));
        assert!(manifest.contains(
            "production generation boundary request corpus summary checksum (fnv1a-64): 0x"
        ));
        assert!(manifest.contains(
        ...
        assert!(manifest
            .contains("lunar theory source window summary: lunar-source-window-summary.txt"));
        ...
        assert!(
            manifest.contains("lunar theory catalog validation summary checksum (fnv1a-64): 0x")
        );
        ...
        assert!(manifest
            .contains("packaged-artifact generation policy summary checksum (fnv1a-64): 0x"));
        ...
        assert!(manifest
            .contains("release-house-system-canonical-names summary checksum (fnv1a-64): 0x"));
        assert!(
            manifest.contains("release-ayanamsa-canonical-names summary checksum (fnv1a-64): 0x")
        );
        ...
        assert!(
            verified.contains("reference-snapshot-2451916-major-body-dense-boundary-summary.txt")
        );
    ...
    fn verify_release_bundle_rejects_tampered_packaged_artifact_generation_policy_summary_even_with_updated_checksum(
        ...
        let bundle_dir =
            unique_temp_dir("pleiades-release-bundle-tampered-generation-policy-semantic");
        let bundle_dir_string = bundle_dir.to_string_lossy().to_string();
        render_cli(&[
            "bundle-release",
            "--out",
    ...
    fn production_generation_source_window_summary_validation_rejects_drift() {
        let summary = production_generation_snapshot_window_summary_for_report();
        let drifted_summary =
            summary.replace("357 source-backed samples", "358 source-backed samples");
    ...
    fn reference_snapshot_source_window_summary_validation_rejects_drift() {
        ...
        let error = ensure_reference_snapshot_source_window_summary_matches_current_rendering(
            &drifted_summary,
        )
    ...
    fn verify_release_bundle_rejects_tampered_validation_report_fit_margin_summary_even_with_updated_checksum(
        ...
        let bundle_dir = unique_temp_dir(
            "pleiades-release-bundle-tampered-validation-report-fit-margin-semantic",
        );
        let bundle_dir_string = bundle_dir.to_string_lossy().to_string();
        render_cli(&[
            "bundle-release",
    ...
    fn production_generation_alias_command_renders_the_overall_block() {
        let rendered = render_cli(&["production-generation"])
    ...
    fn production_generation_boundary_window_alias_rejects_extra_arguments() {
        let error = render_cli(&["production-generation-boundary-window", "extra"]).expect_err(
            "production generation boundary window alias should reject extra arguments",
        );

        assert!(
            error.contains("production-generation-boundary-window does not accept extra arguments")
        );
    }
    ...
    fn reference_snapshot_2451917_major_body_bridge_summary_command_renders_the_bridge_day() {
        ...
        assert_eq!(
            ...
            .expect_err(
                "reference 2451917 major body bridge summary should reject extra arguments"
            ),
    ...
    fn reference_snapshot_boundary_day_summary_command_renders_the_boundary_day() {
        let rendered = render_cli(&["boundary-day-summary"])
    ...
    {
        let rendered =
            render_cli(&["reference-snapshot-2451916-major-body-dense-boundary-summary"]).expect(
                "reference snapshot 2451916 major-body dense boundary summary should render",
            );
    ...
    fn packaged_artifact_production_profile_summary_and_alias_commands_render_the_profile_skeleton()
    {
        ...
        assert_eq!(
            render_cli(&["packaged-artifact-production-profile", "extra"]).expect_err(
                "packaged artifact production profile alias should reject extra arguments"
            ),
    ...
    fn packaged_artifact_target_threshold_summary_and_alias_commands_render_the_summary() {
        ...
        assert_eq!(
            render_cli(&["packaged-artifact-target-threshold-summary", "extra"]).expect_err(
                "packaged artifact target threshold summary should reject extra arguments"
            ),
            "packaged-artifact-target-threshold-summary does not accept extra arguments"
        );
        assert_eq!(
            render_cli(&["packaged-artifact-target-threshold", "extra"]).expect_err(
    ...
    fn packaged_artifact_target_threshold_state_summary_and_alias_commands_render_the_summary() {
        let target_threshold_state =
            render_cli(&["packaged-artifact-target-threshold-state-summary"])
    ...
    fn comparison_and_benchmark_corpus_summary_commands_render_the_corpus_blocks() {
        ...
        assert!(
            ensure_chart_benchmark_corpus_summary_matches_current_rendering(&chart_benchmark)
                .is_ok()
        );
        ...
        let error = ensure_chart_benchmark_corpus_summary_matches_current_rendering(
            &drifted_chart_benchmark,
        )
    ...
    fn reference_snapshot_1750_selected_body_boundary_summary_command_renders_the_selected_body_boundary_block(
        ...
        assert!(
            rendered.contains("Sun, Moon, Mercury, Venus, Mars, Jupiter, Saturn, Uranus, Neptune")
        );
        assert_eq!(
            rendered,
            reference_snapshot_1750_selected_body_boundary_summary_for_report()
    ...
    fn reference_snapshot_2400000_major_body_boundary_summary_command_renders_the_boundary_block() {
        ...
        assert_eq!(alias, rendered);
        assert_eq!(
            render_cli(&["reference-snapshot-2400000-major-body-boundary-summary", "extra"])
                .expect_err("reference snapshot 2400000 major-body boundary summary should reject extra arguments"),
    ...
    fn reference_snapshot_2451545_major_body_boundary_summary_command_renders_the_j2000_block() {
        ...
        assert_eq!(alias, rendered);
        assert_eq!(
            render_cli(&["reference-snapshot-2451545-major-body-boundary-summary", "extra"])
                .expect_err("reference snapshot 2451545 major-body boundary summary should reject extra arguments"),
    ...
    fn reference_snapshot_2500000_major_body_boundary_summary_command_renders_the_terminal_boundary_block(
        ...
        assert_eq!(alias, rendered);
        assert_eq!(
            render_cli(&["reference-snapshot-2500000-major-body-boundary-summary", "extra"])
                .expect_err("reference snapshot 2500000 major-body boundary summary should reject extra arguments"),
    ...
    fn reference_snapshot_2451910_major_body_boundary_summary_aliases_render_the_same_reports() {
        ...
        assert_eq!(
            render_cli(&["reference-snapshot-2451910-major-body-boundary-summary", "extra"])
    ...
    fn reference_snapshot_2451911_major_body_boundary_summary_aliases_render_the_same_reports() {
        ...
        assert_eq!(
            render_cli(&["reference-snapshot-2451911-major-body-boundary-summary", "extra"])
    ...
    fn reference_snapshot_2451912_major_body_boundary_summary_aliases_render_the_same_reports() {
        ...
        assert_eq!(
            render_cli(&["reference-snapshot-2451912-major-body-boundary-summary", "extra"])
    ...
    fn reference_snapshot_2451913_major_body_boundary_summary_aliases_render_the_same_reports() {
        ...
        assert_eq!(
            render_cli(&["reference-snapshot-2451913-major-body-boundary-summary", "extra"])
    ...
    fn reference_snapshot_2451914_major_body_boundary_summary_aliases_render_the_same_reports() {
        ...
        assert_eq!(
            render_cli(&["reference-snapshot-2451914-major-body-boundary-summary", "extra"])
    ...
    fn reference_snapshot_2451915_major_body_boundary_summary_aliases_render_the_same_reports() {
        ...
        assert_eq!(
            render_cli(&["reference-snapshot-2451915-major-body-boundary-summary", "extra"])
    ...
    fn reference_snapshot_2451916_major_body_interior_summary_aliases_render_the_same_reports() {
        ...
        assert_eq!(
            render_cli(&["reference-snapshot-2451916-major-body-interior-summary", "extra"])
    ...
    fn reference_snapshot_2451918_major_body_boundary_summary_aliases_render_the_same_reports() {
        ...
        assert_eq!(
            render_cli(&["reference-snapshot-2451918-major-body-boundary-summary", "extra"])
    ...
    fn reference_snapshot_2451920_major_body_interior_summary_aliases_render_the_same_reports() {
        ...
        assert_eq!(
            render_cli(&["reference-snapshot-2451920-major-body-interior-summary", "extra"])
    ...
    fn reference_snapshot_2360234_major_body_interior_summary_aliases_render_the_same_reports() {
        ...
        assert_eq!(
            render_cli(&["2360234-major-body-interior-summary", "extra"])
                .expect_err(
