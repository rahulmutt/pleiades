...
mod tests {
    ...
    fn default_corpus_covers_the_comparison_snapshot() {
        ...
        assert_eq!(corpus.requests.len(), 43);
        assert_eq!(summary.epoch_count, 7);
        assert_eq!(summary.epochs.len(), 7);
        assert!(summary
            .epochs
            .iter()
    ...
    fn comparison_report_uses_the_snapshot_backend() {
        ...
        assert!(report.lines().any(|line| {
            line == "  epoch labels: JD 2378499.0 (TT), JD 2400000.0 (TT), JD 2451545.0 (TT), JD 2451910.5 (TT), JD 2500000.0 (TT), JD 2600000.0 (TT), JD 2634167.0 (TT)"
        }));
        assert!(report
            .lines()
    ...
    fn validation_report_includes_corpus_metadata() {
        ...
        assert!(report.contains("JPL interpolation quality: 23 samples across 10 bodies"));
    ...
    fn release_notes_command_renders_the_release_notes() {
        ...
        assert!(rendered.contains("Reference snapshot coverage: 63 rows across 15 bodies and 7 epochs (20 asteroid rows; JD 2378499.0 (TDB)..JD 2634167.0 (TDB)); bodies:"));
...
"#;
    ...
    fn release_bundle_writes_expected_artifacts() {
        ...
        assert!(release_summary.contains(
            "JPL reference snapshot equatorial parity: 63 rows across 15 bodies and 7 epochs (JD 2378499.0 (TDB)..JD 2634167.0 (TDB)); bodies:"
        ));
        assert!(release_summary.contains(
