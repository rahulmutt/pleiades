...
mod tests {
    ...
    fn default_corpus_covers_the_comparison_snapshot() {
        let corpus = default_corpus();
        let summary = corpus.summary();
        assert_eq!(corpus.requests.len(), 110);
        assert_eq!(summary.epoch_count, 13);
        assert_eq!(summary.epochs.len(), 13);
        assert!(summary
            .epochs
            .iter()
    ...
    fn comparison_report_uses_the_snapshot_backend() {
        ...
        assert!(report.lines().any(|line| {
            line == "  epoch labels: JD 2360233.5 (TT), JD 2378499.0 (TT), JD 2400000.0 (TT), JD 2451545.0 (TT), JD 2451910.5 (TT), JD 2451911.5 (TT), JD 2451912.5 (TT), JD 2451914.5 (TT), JD 2451916.5 (TT), JD 2453000.5 (TT), JD 2500000.0 (TT), JD 2600000.0 (TT), JD 2634167.0 (TT)"
        }));
        assert!(report
            .lines()
    ...
    fn validation_report_includes_corpus_metadata() {
        ...
        assert!(report.contains("JPL interpolation quality: 100 samples across 10 bodies"));
    ...
    fn release_notes_command_renders_the_release_notes() {
        ...
        assert!(rendered.contains("Reference snapshot coverage: 187 rows across 15 bodies and 17 epochs (55 asteroid rows; JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies:"));
        assert!(rendered.contains("Reference snapshot body-class coverage: major bodies: 132 rows across 10 bodies and 16 epochs; major windows: "));
        assert!(rendered.contains(
    ...
    fn release_notes_summary_command_renders_the_summary() {
        ...
        assert!(rendered.contains("Release-specific coverage:"));
        assert!(rendered.contains("Selected asteroid source evidence: 55 source-backed samples across 5 bodies and 11 epochs (JD 2451545.0 (TDB)..JD 2634167.0 (TDB)); bodies: Ceres, Pallas, Juno, Vesta, asteroid:433-Eros"));
        ...
        assert!(rendered.contains("Reference snapshot source: NASA/JPL Horizons API, DE441, geocentric ecliptic J2000 vector tables.; coverage=major bodies sampled at 1749-12-31 for Sun through Neptune, inner planets sampled across 1800-2500, with an additional 2406 Mars hold-out; major bodies sampled at 1800-01-03 for Sun through Pluto; major bodies sampled at 2400000, 2451545, 2451910.5, 2451916.5, 2451917.5, 2453000.5, and 2500000; Mars sampled at 2600000 and 2634167 for outer boundary coverage; major bodies sampled at 2001-01-02 through 2001-01-05, plus 2001-01-07, for additional boundary coverage; selected asteroids sampled at J2000, 2001-01-01 through 2001-01-07, with 2451915.5 boundary coverage, 2003-12-27, 2132-08-31, and 2500-01-01.; geocentric ecliptic J2000; TDB reference epoch JD 2451545.0 (TDB)"));
        assert!(rendered.contains("Reference snapshot body-class coverage: major bodies: 132 rows across 10 bodies and 16 epochs; major windows: "));
        assert!(rendered.contains(
            "selected asteroids: 55 rows across 5 bodies and 11 epochs; asteroid windows: "
        ));
...
"#;
    ...
    fn release_bundle_writes_expected_artifacts() {
        ...
        assert!(release_summary.contains(
            "JPL reference snapshot equatorial parity: 187 rows across 15 bodies and 17 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies:"
        ));
        assert!(release_summary.contains(
        ...
        assert!(release_summary.contains("Reference snapshot body-class coverage: major bodies: 132 rows across 10 bodies and 16 epochs; major windows: "));
    ...
    fn production_generation_source_window_summary_command_renders_the_source_windows_block() {
        ...
        assert!(rendered.contains("175 source-backed samples"));
        assert_eq!(
            rendered,
            production_generation_snapshot_window_summary_for_report()
        );
    ...
    fn comparison_snapshot_source_window_summary_command_renders_the_source_windows_block() {
        ...
        assert!(rendered.contains("110 source-backed samples"));
        assert_eq!(
            rendered,
    ...
    fn reference_asteroid_source_window_summary_command_renders_the_window_block() {
        ...
        assert!(rendered.contains("source-backed samples across 5 bodies and 11 epochs"));
        assert_eq!(
            rendered,
