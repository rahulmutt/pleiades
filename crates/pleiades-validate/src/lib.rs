...
mod tests {
    ...
    fn release_notes_command_renders_the_release_notes() {
        ...
        assert!(rendered.contains("Reference snapshot coverage: 185 rows across 15 bodies and 16 epochs (55 asteroid rows; JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies:"));
        assert!(rendered.contains("Reference snapshot body-class coverage: major bodies: 130 rows across 10 bodies and 15 epochs; major windows: "));
        assert!(rendered.contains(
...
"#;
    ...
    fn release_bundle_writes_expected_artifacts() {
        ...
        assert!(release_summary.contains(
            "JPL reference snapshot equatorial parity: 185 rows across 15 bodies and 16 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies:"
        ));
        assert!(release_summary.contains(
            "JPL reference snapshot batch parity: 185 rows across 15 bodies and 16 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies:"
        ));
        ...
        assert!(release_summary.contains("Reference snapshot body-class coverage: major bodies: 130 rows across 10 bodies and 15 epochs; major windows: "));
    ...
    fn production_generation_source_window_summary_command_renders_the_source_windows_block() {
        ...
        assert!(rendered.contains("185 source-backed samples"));
        assert_eq!(
            rendered,
            production_generation_snapshot_window_summary_for_report()
        );
