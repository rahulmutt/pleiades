...
/// API stability summary, comparison-envelope summary, validation report summary,
/// artifact summary, packaged-artifact generation manifest, benchmark report,
...
/// comparison-envelope summary, validation report summary, artifact summary,
...
fn ensure_release_bundle_manifest_is_canonical(
    ...
    const EXPECTED_MANIFEST_LINES: [&str; 45] = [
        "Release bundle manifest",
        "profile:",
...
mod tests {
    ...
    fn release_notes_command_renders_the_release_notes() {
        ...
        assert!(rendered.contains("Reference snapshot coverage: 194 rows across 15 bodies and 18 epochs (60 asteroid rows; JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies:"));
    ...
    fn release_notes_summary_command_renders_the_summary() {
        ...
        assert!(rendered.contains("Release-specific coverage:"));
        assert!(rendered.contains("Selected asteroid source evidence: 60 source-backed samples across 5 bodies and 12 epochs (JD 2451545.0 (TDB)..JD 2634167.0 (TDB)); bodies: Ceres, Pallas, Juno, Vesta, asteroid:433-Eros"));
...
"#;
    ...
    fn release_bundle_writes_expected_artifacts() {
        ...
        assert!(release_summary.contains(
            "JPL reference snapshot equatorial parity: 194 rows across 15 bodies and 18 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies:"
        ));
        assert!(release_summary.contains(
            "JPL reference snapshot batch parity: 194 rows across 15 bodies and 18 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies:"
        ));
    ...
    fn production_generation_source_window_summary_command_renders_the_source_windows_block() {
        ...
        assert!(rendered.contains("177 source-backed samples"));
        assert_eq!(
            rendered,
            production_generation_snapshot_window_summary_for_report()
        );
    ...
    fn production_generation_summary_command_renders_the_overall_block() {
        ...
        assert!(rendered.contains("177 rows across 15 bodies and 16 epochs"));
        assert_eq!(
            rendered,
            production_generation_snapshot_summary_for_report()
        );
    ...
    #[test]
    fn reference_high_curvature_summary_command_renders_the_high_curvature_block() {
        let rendered = render_cli(&["reference-high-curvature-summary"])
            .expect("reference high-curvature summary should render");

        assert!(rendered.contains("Reference major-body high-curvature evidence:"));
        assert!(rendered.contains("high-curvature interpolation window"));
        assert_eq!(
            rendered,
            reference_snapshot_high_curvature_summary_for_report()
        );
    ...
    #[test]
    fn reference_major_body_boundary_summary_command_renders_the_boundary_block() {
        let rendered = render_cli(&["reference-snapshot-major-body-boundary-summary"])
            .expect("reference major-body boundary summary should render");

        assert!(rendered.contains("Reference major-body boundary evidence:"));
    ...
    #[test]
    fn reference_mars_jupiter_boundary_summary_command_renders_the_boundary_block() {
        let rendered = render_cli(&["reference-snapshot-mars-jupiter-boundary-summary"])
            .expect("reference Mars/Jupiter boundary summary should render");

        assert!(rendered.contains("Reference Mars/Jupiter boundary evidence:"));
        assert!(rendered.contains("JD 2451918.5 (TDB)"));
        assert_eq!(
            rendered,
            reference_snapshot_mars_jupiter_boundary_summary_for_report()
        );
    ...
    #[test]
    fn reference_mars_outer_boundary_summary_command_renders_the_outer_boundary_block() {
    ...
    #[test]
    fn reference_major_body_boundary_window_summary_command_renders_the_boundary_windows_block() {
    ...
    #[test]
    fn reference_high_curvature_window_summary_command_renders_the_high_curvature_windows_block() {
        let rendered = render_cli(&["reference-high-curvature-window-summary"])
            .expect("reference high-curvature window summary should render");

        assert!(rendered.contains("Reference major-body high-curvature windows:"));
        assert!(rendered.contains("windows:"));
        assert_eq!(
            rendered,
            reference_snapshot_high_curvature_window_summary_for_report()
    ...
    #[test]
    fn reference_snapshot_boundary_epoch_coverage_summary_command_renders_the_boundary_epoch_block()
    {
    ...
    #[test]
    fn source_documentation_summary_command_renders_the_shared_source_documentation_block() {
        let rendered = render_cli(&["source-documentation-summary"])
            .expect("source documentation summary should render");

        assert!(rendered.contains("VSOP87 source documentation:"));
        assert_eq!(rendered, source_documentation_summary_for_report());
    }

    #[test]
    fn source_documentation_health_summary_command_renders_the_shared_health_block() {
        let rendered = render_cli(&["source-documentation-health-summary"])
            .expect("source documentation health summary should render");

        assert!(rendered.contains("VSOP87 source documentation health:"));
        assert_eq!(rendered, source_documentation_health_summary_for_report());
    }

    #[test]
    fn source_audit_summary_command_renders_the_shared_audit_block() {
        let rendered =
            render_cli(&["source-audit-summary"]).expect("source audit summary should render");

        assert!(rendered.contains("VSOP87 source audit:"));
        assert_eq!(rendered, source_audit_summary_for_report());
    }

    #[test]
    fn generated_binary_audit_summary_command_renders_the_shared_audit_block() {
        let rendered = render_cli(&["generated-binary-audit-summary"])
            .expect("generated binary audit summary should render");

        assert!(rendered.contains("VSOP87 generated binary audit:"));
        assert_eq!(rendered, generated_binary_audit_summary_for_report());
    }
    #[test]
    fn time_scale_policy_summary_command_renders_the_shared_time_scale_policy_block() {
        let rendered = render_cli(&["time-scale-policy-summary"])
    ...
    #[test]
    fn delta_t_policy_summary_command_renders_the_shared_delta_t_policy_block() {
        let rendered =
    ...
    #[test]
    fn packaged_lookup_epoch_policy_summary_command_renders_the_shared_lookup_epoch_policy_block() {
        let rendered = render_cli(&["packaged-lookup-epoch-policy-summary"])
            .expect("packaged lookup epoch policy summary should render");

        assert!(rendered.contains("TT-grid retag without relativistic correction"));
        assert_eq!(rendered, packaged_lookup_epoch_policy_summary_for_report());
    }

    #[test]
    fn artifact_profile_coverage_summary_command_renders_the_shared_profile_coverage_block() {
        let rendered = render_cli(&["artifact-profile-coverage-summary"])
            .expect("artifact profile coverage summary should render");

        assert!(rendered.contains("Artifact profile coverage: "));
        assert!(rendered.contains("asteroid:433-Eros"));
        assert_eq!(
            rendered,
            format!(
    ...
    #[test]
    fn artifact_boundary_envelope_summary_command_renders_the_shared_boundary_block() {
        let rendered = render_cli(&["artifact-boundary-envelope-summary"])
    ...
    #[test]
    fn packaged_artifact_output_support_summary_command_renders_the_shared_output_support_block() {
        let rendered = render_cli(&["packaged-artifact-output-support-summary"])
            .expect("packaged artifact output support summary should render");

        assert!(rendered.contains("Packaged-artifact output support: "));
        assert!(rendered.contains("ApparentCorrections=unsupported"));
        assert_eq!(
            rendered,
            format!(
                "Packaged-artifact output support: {}",
    ...
    #[test]
    fn packaged_artifact_storage_summary_command_renders_the_shared_storage_block() {
        let rendered = render_cli(&["packaged-artifact-storage-summary"])
            .expect("packaged artifact storage summary should render");

        assert!(rendered.contains("Packaged-artifact storage/reconstruction: "));
        assert!(rendered.contains("equatorial coordinates are reconstructed"));
        assert_eq!(
            rendered,
            format!(
                "Packaged-artifact storage/reconstruction: {}",
    ...
    #[test]
    fn packaged_artifact_regeneration_summary_command_renders_the_shared_regeneration_block() {
        let rendered = render_cli(&["packaged-artifact-regeneration-summary"])
            .expect("packaged artifact regeneration summary should render");

        assert!(rendered.contains("Packaged-artifact regeneration: "));
        assert!(rendered.contains("profile id="));
        assert_eq!(
            rendered,
            format!(
                "Packaged-artifact regeneration: {}",
    ...
    #[test]
    fn packaged_artifact_target_threshold_summary_command_renders_the_shared_threshold_block() {
        let rendered = render_cli(&["packaged-artifact-target-threshold-summary"])
            .expect("packaged artifact target threshold summary should render");

        assert!(rendered.contains("Packaged-artifact target thresholds: "));
        assert_eq!(
            rendered,
            format!(
                "Packaged-artifact target thresholds: {}",
    ...
    #[test]
    fn packaged_artifact_target_threshold_scope_envelopes_summary_command_renders_the_shared_scope_block(
    ) {
        let rendered = render_cli(&["packaged-artifact-target-threshold-scope-envelopes-summary"])
            .expect("packaged artifact target threshold scope envelopes summary should render");

        assert!(rendered.contains("Packaged-artifact target-threshold scope envelopes: "));
        assert_eq!(
            rendered,
            format!(
                "Packaged-artifact target-threshold scope envelopes: {}",
    ...
    #[test]
    fn packaged_artifact_generation_policy_summary_command_renders_the_shared_policy_block() {
        let rendered = render_cli(&["packaged-artifact-generation-policy-summary"])
    ...
    #[test]
    fn packaged_artifact_generation_residual_bodies_summary_command_renders_the_shared_residual_body_block(
    ) {
        let rendered = render_cli(&["packaged-artifact-generation-residual-bodies-summary"])
    ...
    #[test]
    fn interpolation_posture_summary_command_renders_the_shared_interpolation_posture_block() {
        let rendered = render_cli(&["interpolation-posture-summary"])
            .expect("interpolation posture summary should render");

        assert!(rendered.contains("Interpolation posture summary"));
        assert!(rendered.contains("Interpolation posture: JPL interpolation posture: source=leave-one-out runtime interpolation evidence derived from the checked-in reference snapshot; detail=transparency evidence only; envelope=not a production tolerance envelope"));
    }

    #[test]
    fn interpolation_quality_summary_command_renders_the_shared_interpolation_quality_block() {
        let rendered = render_cli(&["interpolation-quality-summary"])
            .expect("interpolation quality summary should render");
    ...
    #[test]
    fn interpolation_quality_kind_coverage_summary_command_renders_the_shared_interpolation_quality_kind_coverage_block(
    ) {
        let rendered = render_cli(&["interpolation-quality-kind-coverage-summary"])
    ...
    #[test]
    fn lunar_reference_error_envelope_summary_command_renders_the_shared_lunar_reference_error_envelope_block(
    ) {
        let rendered = render_cli(&["lunar-reference-error-envelope-summary"])
    ...
    #[test]
    fn lunar_equatorial_reference_error_envelope_summary_command_renders_the_shared_lunar_equatorial_reference_error_envelope_block(
    ) {
        let rendered = render_cli(&["lunar-equatorial-reference-error-envelope-summary"])
    ...
    #[test]
    fn lunar_apparent_comparison_summary_command_renders_the_shared_lunar_apparent_comparison_block(
    ) {
        let rendered = render_cli(&["lunar-apparent-comparison-summary"])
    ...
    #[test]
    fn lunar_source_window_summary_command_renders_the_shared_lunar_source_window_block() {
        let rendered = render_cli(&["lunar-source-window-summary"])
            .expect("lunar source window summary should render");

        assert!(rendered.contains("lunar source windows"));
        assert_eq!(rendered, lunar_source_window_summary_for_report());
    }

    #[test]
    fn selected_asteroid_boundary_summary_command_renders_the_selected_asteroid_boundary_block() {
        let rendered = render_cli(&["selected-asteroid-boundary-summary"])
    ...
    #[test]
    fn selected_asteroid_terminal_boundary_summary_command_renders_the_terminal_boundary_block() {
        let rendered =
        ...
        assert!(rendered.contains("terminal boundary evidence"));
    ...
    #[test]
    fn selected_asteroid_source_evidence_summary_command_renders_the_selected_asteroid_block() {
    ...
    #[test]
    fn selected_asteroid_source_window_summary_command_renders_the_selected_asteroid_window_block()
    {
    ...
    #[test]
    fn selected_asteroid_batch_parity_summary_command_renders_the_mixed_frame_block() {
        let rendered = render_cli(&["selected-asteroid-batch-parity-summary"])
            .expect("selected asteroid batch parity summary should render");

        assert!(rendered.contains("Selected asteroid batch parity"));
        assert!(rendered.contains("frame mix: 3 ecliptic, 2 equatorial"));
        assert_eq!(
            rendered,
            selected_asteroid_batch_parity_summary_for_report()
        );
    ...
    #[test]
    fn reference_asteroid_evidence_summary_command_renders_the_reference_asteroid_block() {
        let rendered = render_cli(&["reference-asteroid-evidence-summary"])
            .expect("reference asteroid evidence summary should render");

        assert!(rendered.contains("Selected asteroid evidence:"));
        assert!(rendered.contains("Ceres"));
        assert_eq!(rendered, reference_asteroid_evidence_summary_for_report());
    }

    #[test]
    fn reference_asteroid_equatorial_evidence_summary_command_renders_the_equatorial_block() {
    ...
    #[test]
    fn reference_asteroid_source_window_summary_command_renders_the_window_block() {
        ...
        assert!(rendered.contains("source-backed samples across 5 bodies and 12 epochs"));
        assert_eq!(
            rendered,
    ...
    #[test]
    fn jpl_batch_error_taxonomy_summary_command_renders_the_current_batch_taxonomy() {
        let rendered = render_cli(&["jpl-batch-error-taxonomy-summary"])
    ...
    #[test]
    fn jpl_snapshot_evidence_summary_command_renders_the_combined_evidence_block() {
        let rendered = render_cli(&["jpl-snapshot-evidence-summary"])
            .expect("JPL snapshot evidence summary should render");

        assert!(rendered.contains("Reference snapshot coverage:"));
        assert!(rendered.contains("Reference major-body boundary evidence:"));
        assert_eq!(rendered, jpl_snapshot_evidence_summary_for_report());
    }

    #[test]
    fn request_surface_summary_command_renders_the_inventory_block() {
        let rendered = render_cli(&["request-surface-summary"])
            .expect("request surface summary should render");

        assert!(rendered.contains("Request surface summary"));
        assert!(rendered.contains("Primary request surfaces:"));
        assert!(rendered
        ...
        assert!(rendered.contains("pleiades-cli chart (explicit --tt|--tdb|--utc|--ut1 flags plus caller-supplied TT/TDB offset aliases: --tt-offset-seconds, --tt-from-utc-offset-seconds, --tt-from-ut1-offset-seconds, --tdb-offset-seconds, --tdb-from-utc-offset-seconds, --tdb-from-ut1-offset-seconds, --tdb-from-tt-offset-seconds, and --tt-from-tdb-offset-seconds)"));
        assert_eq!(
            rendered,
            format!(
                "Request surface summary\n{}\n",
    ...
    #[test]
    fn request_policy_summary_command_renders_the_shared_request_semantics_block() {
        let rendered =
    ...
    #[test]
    fn lunar_theory_summary_commands_render_the_shared_lunar_theory_blocks() {
        ...
        assert_eq!(lunar_theory_summary, lunar_theory_summary_for_report());
    ...
    #[test]
    fn comparison_envelope_summary_command_renders_the_shared_envelope_block() {
        let rendered = render_cli(&["comparison-envelope-summary"])
        ...
        .expect("comparison envelope summary should build");
        let envelope = comparison_envelope_summary(&comparison.summary, &comparison.samples);

        assert!(rendered.contains("Comparison envelope summary"));
        assert!(rendered.contains("Summary line: "));
        assert!(rendered.contains("Percentile line: "));
        assert_eq!(rendered, render_comparison_envelope_summary_text());
        assert_eq!(
            rendered,
            format!(
                "Comparison envelope summary\nSummary line: {}\nPercentile line: {}\n",
                envelope
                    .validated_summary_line(&comparison.samples)
                    .expect("comparison envelope summary should validate"),
                envelope
                    .validated_percentile_line(&comparison.samples)
                    .expect("comparison percentile summary should validate")
            )
        );
    }

    #[test]
    fn pluto_fallback_summary_command_renders_the_shared_pluto_scope_block() {
        let rendered =
            render_cli(&["pluto-fallback-summary"]).expect("Pluto fallback summary should render");
        assert!(rendered.contains("Pluto fallback summary"));
        assert!(rendered.contains("Pluto fallback: Pluto fallback (approximate): Δlon≤45.000°, Δlat≤1.000°, Δdist=0.250 AU"));
    }
