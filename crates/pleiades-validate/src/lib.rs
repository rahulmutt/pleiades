...
mod tests {
    ...
    fn default_corpus_covers_the_comparison_snapshot() {
        let corpus = default_corpus();
        let summary = corpus.summary();
        assert_eq!(corpus.requests.len(), 90);
        assert_eq!(summary.epoch_count, 11);
        assert_eq!(summary.epochs.len(), 11);
        assert!(summary
            .epochs
            .iter()
    ...
    fn comparison_report_uses_the_snapshot_backend() {
        ...
        assert!(report.lines().any(|line| {
            line == "  epoch labels: JD 2360233.5 (TT), JD 2378499.0 (TT), JD 2400000.0 (TT), JD 2451545.0 (TT), JD 2451911.5 (TT), JD 2451912.5 (TT), JD 2451914.5 (TT), JD 2453000.5 (TT), JD 2500000.0 (TT), JD 2600000.0 (TT), JD 2634167.0 (TT)"
        }));
        assert!(report
            .lines()
    ...
    fn validation_report_includes_corpus_metadata() {
        ...
        assert_report_contains_exact_line(
            &report,
            "  Comparison snapshot coverage: 90 rows across 10 bodies and 11 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies: Sun, Moon, Mercury, Venus, Mars, Jupiter, Saturn, Uranus, Neptune, Pluto",
        );
        assert!(report.contains("Apparentness: Mean"));
        ...
        assert!(report.contains("JPL interpolation quality: 80 samples across 10 bodies"));
    ...
    fn release_notes_command_renders_the_release_notes() {
        ...
        assert!(rendered.contains("Reference snapshot coverage: 149 rows across 15 bodies and 14 epochs (49 asteroid rows; JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies:"));
...
"#;
    ...
    fn release_bundle_writes_expected_artifacts() {
        ...
        assert!(release_summary.contains(
            "JPL reference snapshot equatorial parity: 149 rows across 15 bodies and 14 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies:"
        ));
        assert!(release_summary.contains(
            "JPL reference snapshot batch parity: 149 rows across 15 bodies and 14 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies:"
        ));
    ...
    fn production_generation_source_window_summary_command_renders_the_source_windows_block() {
        ...
        assert!(rendered.contains("151 source-backed samples"));
        assert_eq!(
            rendered,
            production_generation_snapshot_window_summary_for_report()
        );
    ...
    fn comparison_snapshot_source_window_summary_command_renders_the_source_windows_block() {
        ...
        assert!(rendered.contains("90 source-backed samples"));
        assert_eq!(
            rendered,
