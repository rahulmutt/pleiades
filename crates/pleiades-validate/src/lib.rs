...
struct ParsedReleaseBundleManifest {
    ...
    release_profile_identifiers_summary_path: String,
    release_profile_identifiers_summary_checksum: u64,
...
mod tests {
    ...
    fn default_corpus_covers_the_comparison_snapshot() {
        let corpus = default_corpus();
        let summary = corpus.summary();
        assert_eq!(corpus.requests.len(), 131);
        assert_eq!(summary.epoch_count, 16);
        assert_eq!(summary.epochs.len(), 16);
        assert!(summary
            .epochs
            .iter()
    ...
    fn comparison_report_uses_the_snapshot_backend() {
        ...
        assert!(report.lines().any(|line| {
            line == "  epoch labels: JD 2360233.5 (TT), JD 2378499.0 (TT), JD 2400000.0 (TT), JD 2451545.0 (TT), JD 2451910.5 (TT), JD 2451911.5 (TT), JD 2451912.5 (TT), JD 2451914.5 (TT), JD 2451915.0 (TT), JD 2451915.5 (TT), JD 2451916.5 (TT), JD 2451918.5 (TT), JD 2453000.5 (TT), JD 2500000.0 (TT), JD 2600000.0 (TT), JD 2634167.0 (TT)"
        }));
        assert!(report
            .lines()
    ...
    fn validation_report_includes_corpus_metadata() {
        ...
        assert!(report.contains("JPL interpolation quality: 121 samples across 10 bodies"));
    ...
    fn validation_report_summary_renders_a_compact_overview() {
        ...
        assert!(report.contains("House validation corpus"));
        assert!(report.contains("House validation corpus: 7 scenarios"));
        assert!(report.contains("Comparison summary"));
        assert!(report.contains("95th percentile absolute deltas:"));
    ...
    fn cli_help_lists_the_validation_commands() {
        ...
        assert!(rendered.contains("request-semantics        Alias for request-policy-summary"));
    ...
    fn release_notes_command_renders_the_release_notes() {
        ...
        assert!(rendered.contains("Reference snapshot coverage: 232 rows across 15 bodies and 19 epochs (71 asteroid rows; JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies:"));
        assert!(rendered.contains("Reference snapshot body-class coverage: major bodies: 161 rows across 10 bodies and 19 epochs; major windows: "));
    ...
    fn house_validation_summary_command_renders_the_summary() {
        ...
        assert!(rendered.contains("House validation corpus: 7 scenarios"));
        assert!(rendered
    ...
    fn release_notes_summary_command_renders_the_summary() {
        ...
        assert!(rendered.contains("Release-specific coverage:"));
        assert!(rendered.contains("Selected asteroid source evidence: 71 source-backed samples across 5 bodies and 15 epochs (JD 2378498.5 (TDB)..JD 2634167.0 (TDB)); bodies: Ceres, Pallas, Juno, Vesta, asteroid:433-Eros"));
...
"#;
    ...
    fn release_bundle_writes_expected_artifacts() {
        ...
        assert!(release_summary.contains(
            "JPL reference snapshot equatorial parity: 232 rows across 15 bodies and 19 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies:"
        ));
        assert!(release_summary.contains(
        ...
        assert!(release_summary.contains("Reference snapshot body-class coverage: major bodies: 161 rows across 10 bodies and 19 epochs; major windows: "));
        ...
        assert!(artifact_summary.contains("residual-bearing segments: 8"));
    ...
    fn production_generation_summary_command_renders_the_overall_block() {
        ...
        assert!(rendered.contains("232 rows across 15 bodies and 19 epochs"));
        assert_eq!(
            rendered,
            production_generation_snapshot_summary_for_report()
        );
    ...
    fn comparison_snapshot_source_window_summary_command_renders_the_source_windows_block() {
        ...
        assert!(rendered.contains("131 source-backed samples"));
        assert_eq!(
            rendered,
