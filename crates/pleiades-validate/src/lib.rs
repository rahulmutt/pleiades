...
mod tests {
    ...
    fn default_corpus_covers_the_comparison_snapshot() {
        let corpus = default_corpus();
        let summary = corpus.summary();
        assert_eq!(corpus.requests.len(), 122);
        assert_eq!(summary.epoch_count, 15);
        assert_eq!(summary.epochs.len(), 15);
        assert!(summary
            .epochs
            .iter()
    ...
    fn comparison_report_uses_the_snapshot_backend() {
        ...
        assert!(report.lines().any(|line| {
            line == "  epoch labels: JD 2360233.5 (TT), JD 2378499.0 (TT), JD 2400000.0 (TT), JD 2451545.0 (TT), JD 2451910.5 (TT), JD 2451911.5 (TT), JD 2451912.5 (TT), JD 2451914.5 (TT), JD 2451915.0 (TT), JD 2451916.5 (TT), JD 2451918.5 (TT), JD 2453000.5 (TT), JD 2500000.0 (TT), JD 2600000.0 (TT), JD 2634167.0 (TT)"
        }));
        assert!(report
            .lines()
    ...
    fn validation_report_includes_corpus_metadata() {
        ...
        assert!(report.contains("JPL interpolation quality: 112 samples across 10 bodies"));
    ...
    fn release_notes_command_renders_the_release_notes() {
        ...
        assert!(rendered.contains("Reference snapshot coverage: 210 rows across 15 bodies and 19 epochs (66 asteroid rows; JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies:"));
        assert!(rendered.contains("Reference snapshot body-class coverage: major bodies: 144 rows across 10 bodies and 18 epochs; major windows: "));
        assert!(rendered.contains(
...
"#;
    ...
    fn release_bundle_writes_expected_artifacts() {
        ...
        assert!(release_summary.contains(
            "JPL reference snapshot equatorial parity: 210 rows across 15 bodies and 19 epochs (JD 2360233.5 (TDB)..JD 2634167.0 (TDB)); bodies:"
        ));
        assert!(release_summary.contains(
        ...
        assert!(release_summary.contains("Reference snapshot body-class coverage: major bodies: 144 rows across 10 bodies and 18 epochs; major windows: "));
    ...
    fn production_generation_summary_command_renders_the_overall_block() {
        ...
        assert!(rendered.contains("210 rows across 15 bodies and 19 epochs"));
        assert_eq!(
            rendered,
            production_generation_snapshot_summary_for_report()
        );
    ...
    fn comparison_snapshot_source_window_summary_command_renders_the_source_windows_block() {
        ...
        assert!(rendered.contains("122 source-backed samples"));
        assert_eq!(
            rendered,
