...
fn cli_report_summary_lists_the_summary_command() {
    ...
    assert!(validation_report_summary.contains(
            "Release profile identifiers: v1 compatibility=pleiades-compatibility-profile/0.7.13, api-stability=pleiades-api-stability/0.2.2"
        ));
    assert!(validation_report_summary
...
fn request_policy_summary_and_alias_commands_render_the_policy_block() {
    ...
    assert_eq!(
            request_semantics,
            format!(
                "{}Unsupported modes: built-in UTC convenience remains out of scope; built-in Delta T remains out of scope; native sidereal backend output remains unsupported unless a backend explicitly advertises it\n",
