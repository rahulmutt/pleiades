...
fn release_notes_summary_command_renders_the_summary() {
    ...
    assert!(rendered.contains("API stability summary line: API stability posture: pleiades-api-stability/0.3.0; stable surfaces: 8; experimental surfaces: 3; deprecation policy items: 4; intentional limits: 3"));
    assert!(rendered.contains(&reference_snapshot_source_summary_for_report()));
...
fn release_summary_command_renders_the_quick_overview() {
    ...
    assert!(rendered.contains("Time-scale policy: direct backend requests accept TT/TDB; civil UTC/UT1 inputs convert via the pleiades-time crate or caller-supplied offsets; the ephemeris backends carry no internal Delta T or UTC convenience model"));
    ...
    assert!(rendered.contains("Apparentness policy: backends remain mean-only and J2000 at the backend boundary; apparent place of date (chart layer, default): light-time + precession-to-date + annual aberration + nutation-in-longitude, release-grade bodies; gravitational light-deflection omitted"));
