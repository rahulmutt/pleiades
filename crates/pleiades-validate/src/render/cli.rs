...
/// Drop a 7th `pleiades_jd_tdb` golden column back to the 6-column SE form:
/// truncates data rows to 6 fields and strips the header's `,pleiades_jd_tdb`
/// suffix. Comment/blank lines pass through unchanged. Used by `crossings-golden`
/// so regeneration is idempotent (it always starts from the SE reference columns).
...
pub fn render_cli(args: &[&str]) -> Result<String, String> {
    match args.first().copied() {
        ...
        Some("validate-topocentric") | Some("topocentric-gate") => {
            ensure_no_extra_args(&args[1..], "validate-topocentric")?;
        ...
        Some("validate-angles") | Some("angles-gate") => {
            ensure_no_extra_args(&args[1..], "validate-angles")?;
        ...
        Some("validate-rise-trans") | Some("rise-trans-gate") => {
            ensure_no_extra_args(&args[1..], "validate-rise-trans")?;
        ...
        Some("eclipse-local") => {
            crate::eclipse_local_validation::render_eclipse_local_listing(&args[1..])
        }
        ...
        Some("packaged-artifact-latency-budget-summary") | Some("artifact-latency") => {
            ...
            Ok(render_packaged_artifact_latency_budget_summary())
        }
...
fn render_ingest_public(args: &[&str]) -> Result<String, String> {
    ...
    use pleiades_jpl::ingest::{ExpectedProfile, InputFormat, Provenance};

    // For this phase the CLI asserts nothing; the profile is all-silent.
    let expected = ExpectedProfile::default();

    let out = match format {
        None => pleiades_jpl::ingest::read_public_corpus_from_path(input, &expected),
        Some(name) => {
            let fmt = match name {
...
mod ingest_public_tests {
    ...
    fn ingest_public_reports_detected_format_and_counts() {
        ...
        assert!(
            out.contains("rows: 2") || out.contains("entries: 2"),
            "got: {out}"
        );
...
";
    ...
    #[test]
    fn run_all_numeric_gates_includes_eclipses_local_and_passes() {
        // `validate_eclipse_local_corpus` is called directly (not just via
        // `run_all_numeric_gates`) to pin that the local-eclipse gate itself
    ...
    fn run_all_numeric_gates_includes_fictitious_and_passes() {
        // `validate_fictitious_corpus` is called directly (not just via
        // `run_all_numeric_gates`) to pin that the fictitious-body gate itself
        // passes independently of the rest of the battery, in addition to the
        // battery-level assertion above.
    ...
    #[test]
    fn run_all_numeric_gates_includes_nod_aps_and_passes() {
        // `validate_nod_aps_corpus` is called directly (not just via
        // `run_all_numeric_gates`) to pin that the nod-aps gate itself
        // passes independently of the rest of the battery, in addition to the
        // battery-level assertion above.
    ...
    #[test]
    fn run_all_numeric_gates_includes_pheno_and_passes() {
        // `validate_pheno_corpus` is called directly (not just via
        // `run_all_numeric_gates`) to pin that the pheno gate itself passes
