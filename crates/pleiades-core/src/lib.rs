...
pub use compatibility::{
    catalog_inventory_summary_for_report, compatibility_caveats_summary_for_report,
    current_compatibility_profile, current_compatibility_profile_id,
    custom_definition_ayanamsa_labels_summary_for_report, house_code_aliases_summary_for_report,
    house_formula_families_summary_for_report,
    latitude_sensitive_house_constraints_summary_for_report,
...
mod tests {
    ...
    #[test]
    fn validate_chart_request_and_batch_preserve_invalid_observer_before_unsupported_body() {
        let engine = ChartEngine::new(SimpleBackend);
        let instant = Instant::new(JulianDay::from_days(2451545.0), TimeScale::Tt);
        let invalid_observer_request = ChartRequest::new(instant)
        ...
        let single_error = engine
            .validate_chart_request(&invalid_observer_request)
            .expect_err(
                "chart validation should reject malformed observers before body coverage checks",
            );
        assert_eq!(single_error.kind, EphemerisErrorKind::InvalidObserver);
        assert!(single_error
            .message
