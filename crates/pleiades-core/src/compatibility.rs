...
pub const fn current_compatibility_profile() -> CompatibilityProfile {
    CompatibilityProfile {
        ...
        known_gaps: &[
            ...
            "The Babylonian house-family labels (Babylonian (House), Babylonian (Sissy), Babylonian (True Geoc), Babylonian (True Topc), Babylonian (True Obs), and Babylonian (House Obs)) remain catalogued without published sidereal zero-point metadata in this release line; they are the clearest open ayanamsa gap and stay on the explicit-gap list until a source-backed mapping is added or the catalog is widened again.",
...
mod tests {
    ...
    fn profile_includes_baseline_and_release_catalogs() {
        ...
        assert!(profile
            .known_gaps
            .iter()
            .any(|gap| gap.contains("Babylonian (House)")));
        assert!(profile
            .known_gaps
            .iter()
            .any(|gap| gap.contains("House Obs")));
    }
