...
pub const fn current_compatibility_profile() -> CompatibilityProfile {
    CompatibilityProfile {
        ...
        release_notes: &[
            "Release-specific house-system additions now include Equal (MC), Equal (1=Aries), Vehlow Equal, Sripati, Carter (poli-equatorial), Horizon/Azimuth, APC, Krusinski-Pisa-Goelzer, Albategnius, Pullen SD, Pullen SR, Sunshine, and Gauquelin sectors, with the Whole Sign (house 1 = Aries) label, Wang alias, Equal MC / Equal Midheaven aliases, APC / Ascendant Parallel Circle aliases, and Horizon / Horizontal / Azimuthal aliases also resolving as interoperability aliases.",
