...
pub const fn current_compatibility_profile() -> CompatibilityProfile {
    CompatibilityProfile {
        ...
        release_notes: &[
            ...
            "SP-2a (longitude crossings): a new pleiades-events crate ships a longitude-crossing engine — CrossingEngine with next_sun_crossing/next_moon_crossing (Swiss-Ephemeris solcross/mooncross analogues), general geocentric-apparent-of-date body crossings, and heliocentric helio_cross crossings — over the 1900-2100 TDB window, exposed via the validate-crossings CLI (aliases crossings / crossings-gate) and not re-exported from pleiades-core. The fail-closed validate-crossings gate recomputes 41 Swiss-Ephemeris reference crossings and is wired into the release gate set. Honesty caveat: the SE reference is computed with Moshier theory while the engine backend uses VSOP87/ELP, so the gate's per-body time ceilings (Sun/Moon 60 s, planet 2800 s, heliocentric 7200 s) are cross-theory-calibrated placeholders pending maintainer review — they absorb an irreducible cross-theory floor and are NOT a claim of tight arcsecond Swiss-Ephemeris parity (precedent: validate-lilith accepts an SE-vs-ours residual of ~306\").",
        ],
