...
pub const fn current_compatibility_profile() -> CompatibilityProfile {
    CompatibilityProfile {
        ...
        release_notes: &[
            ...
            "SP-6 (lunar occultations) additions: pleiades-events ships a Swiss-Ephemeris swe_lun_occult_* analogue set — EventEngine::occultation (local circumstances for a given observer — the \"how\" analogue; Swiss Ephemeris has no separate swe_lun_occult_how call, so its magnitude/obscuration/contacts are validated against swe_lun_occult_when_loc's returned attributes), EventEngine::next_occultation/previous_occultation (per-observer search for the next/previous occultation, swe_lun_occult_when_loc analogues), and EventEngine::next_global_occultation (global search, a swe_lun_occult_when_glob analogue, its sub-lunar point localized via swe_lun_occult_where) — for the Moon occulting planets Mercury-Pluto and curated fixed stars. The fail-closed validate-occultations gate (CLI aliases validate-occultations, occultations, occult-gate), wired into run_all_numeric_gates, checks SE parity against a committed 62-row Swiss-Ephemeris reference corpus, checksum-guarded (fnv1a64) and pinned by row count. Verified coverage: contact/max instants, occultation type (Total/Grazing/Miss), star magnitude and obscuration (exact SE parity), planet magnitude (relative SE parity), the global sub-lunar (central-observation) point, and planet-grazing obscuration. SP-6-FU (central axis-pierce exactness + graze-boundary root cause) additions: central is now the exact Swiss-Ephemeris SE_ECL_CENTRAL axis-pierce condition (an eclipse_where closed-form perpendicular-distance-to-axis test), fully decoupled from occ_type, and hard-gated exact against the committed corpus on both planet (0/6 mismatched) and star (0/12 mismatched) glob rows. Known bound: central-path cartography and exhaustive/global (planet-total) obscuration remain out of scope for this gate — they are different quantities from what is checked. The graze-boundary classification disagreements were root-caused (SP-6-FU differential harness + SE source read) to SE's swe_lun_occult_when_loc folding VISIBILITY (target apparent altitude > 0 at one of max/C1..C4) into event existence, while this engine reports geometry (occultation_type) and visibility (any_phase_visible) separately; the validation gate now compares like-for-like (SE-equivalent Miss = geometric Miss OR no phase visible), reducing disagreements 8/18 to 3/18 (the residual 3 = 1 knife-edge + 2 attributed to the continuous-scan vs discrete-instant visibility sampling delta), pinned fail-closed at 3. Engine numerics were exonerated (Moon position agrees with de440 to ~0.0001″). Compatibility profile bumped to 0.7.13; API stability profile unchanged at 0.2.2 (pleiades-events is unpublished; the occultation surface is purely additive).",
        ],
