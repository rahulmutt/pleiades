...
pub const CURRENT_COMPATIBILITY_PROFILE_ID: &str = "pleiades-compatibility-profile/0.7.2";
...
pub const fn current_compatibility_profile() -> CompatibilityProfile {
    CompatibilityProfile {
        ...
        release_notes: &[
            ...
            "SP-1 (angles and sidereal time): public sidereal-time helpers (GMST/GAST/local via pleiades_apparent::sidereal_time and SiderealTime, plus greenwich_mean_sidereal_time_degrees and equation_of_equinoxes_degrees) and AscMc chart-point extras (ARMC, Vertex, equatorial ascendant, co-ascendants, polar ascendant via pleiades_houses::AscMc, chart_points, and chart_points_from_armc) are now part of the stable chart surface; HouseSnapshot::asc_mc carries AscMc on every house snapshot; HouseSnapshot is now #[non_exhaustive] as a deliberate one-time 0.2.x breaking change; ChartSnapshot::asc_mc() re-exposes AscMc at the facade layer; the validate-angles numeric gate is wired into run_all_numeric_gates.",
        ],
