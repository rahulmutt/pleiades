...
pub(crate) const CURRENT_API_STABILITY_PROFILE_ID: &str = "pleiades-api-stability/0.3.0";
...
pub const fn current_api_stability_profile() -> ApiStabilityProfile {
    ApiStabilityProfile {
        ...
        stable_surfaces: &[
            ...
            "pleiades-apparent's sidereal_time function and SiderealTime type (Greenwich Mean Sidereal Time, Greenwich Apparent Sidereal Time, local sidereal time) are stable façade helpers for Earth-rotation-based time; greenwich_mean_sidereal_time_degrees, equation_of_equinoxes_degrees, and the shared equation_of_equinoxes(delta_psi_deg, true_obliquity_deg) helper are stable degree-valued conveniences. Sidereal time is UT1-based (Earth rotation); the Instant JD is consumed as supplied.",
        ...
        experimental_surfaces: &[
            "pleiades-validate report text, release bundle layout, and validation-corpus composition remain operational tooling rather than a public interchange format; relocated policy/report posture prose (posture module) is part of that operational tooling",
