...
mod tests {
    ...
    fn summary_commands_render_compact_reports() {
        ...
        assert!(release_notes_summary.lines().any(|line| {
            line == "Packaged-artifact generation policy: adjacent same-body linear spans; bodies with a single sampled epoch use point segments; bodies with two or more sampled epochs are fit with linear spans between consecutive sampled epochs"
        }));
