...
//! source file.
//!
//! Neptune now uses a generated binary table derived from the vendored public
//! IMCCE/CELMECH VSOP87B source file, mirroring the Earth/Sun, Mercury, Venus,
//! Mars, Jupiter, Saturn, and Uranus paths so the backend keeps a source-backed
//! path for every supported VSOP87 major planet while Pluto remains outside the
...
mod tests {
    ...
    #[test]
    fn parses_generated_neptune_table_blob_with_expected_series_counts() {
        let tables = parse_generated_vsop87b_tables(include_bytes!("../data/VSOP87B.nep.bin"));
