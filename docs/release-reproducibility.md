...
The CI gate also runs a workspace-native dependency audit that checks manifests, the pinned `mise.toml` toolchain, lockfile entries, and crate-root `build.rs` files:
...
The repository also ships a release-style smoke check that exercises the validation bundle path end to end, plus a full release gate for the maintainer workflow:
...
That smoke check first runs the workspace audit, validates the bundled compressed artifact, then generates a temporary release bundle and verifies the staged manifest checksums and manifest checksum sidecar using `pleiades-validate`. The release gate layers in format, clippy, tests, and benchmark generation before it runs that smoke path.
...
# or: cargo run -q -p pleiades-validate -- bundle-release --output /tmp/pleiades-release
```

The staged bundle now also includes `lunar-theory-source-family-summary.txt` and `lunar-theory-source-selection-summary.txt` alongside the lunar-theory limitations and catalog validation summaries.

The validation tool can also render the full compatibility profile, the compact profile summary, verify the release compatibility profile against the canonical catalogs, render the release notes, release-notes summary, release checklist, release-checklist summary, release summary, comparison-envelope summary, comparison-corpus release-guard summary / comparison-corpus-release-guard / comparison-corpus-guard-summary / comparison-corpus-guard, reference-holdout-overlap-summary / holdout-overlap-summary, production-generation-boundary-summary, reference-snapshot-major-body-bridge-summary, reference-snapshot-bridge-day-summary / bridge-day-summary, selected-asteroid-bridge-summary, frame-policy-summary, observer-policy-summary, apparentness-policy-summary, time-scale-policy-summary, utc-convenience-policy-summary / utc-convenience-policy, delta-t-policy-summary, request-surface-summary / request-surface, catalog-inventory-summary / catalog-inventory, house-validation-summary, house-formula-families-summary, house-code-aliases-summary, ayanamsa-catalog-validation-summary, ayanamsa-metadata-coverage-summary, ayanamsa-reference-offsets-summary, release-house-system-canonical-names-summary / release-house-system-canonical-names, release-ayanamsa-canonical-names-summary / release-ayanamsa-canonical-names, source-documentation-summary / source-documentation, source-documentation-health-summary / source-documentation-health, request-policy-summary / request-policy / request-semantics-summary / request-semantics, compatibility-caveats-summary / compatibility-caveats, native-sidereal-policy-summary / native-sidereal-policy, lunar-theory-limitations-summary / lunar-theory-limitations, lunar-theory-source-family-summary / lunar-theory-source-family, lunar-theory-source-selection-summary / lunar-theory-source-selection, interpolation-posture-summary / interpolation-posture, packaged-lookup-epoch-policy-summary / packaged-lookup-epoch-policy, API stability summary, backend-matrix-summary, artifact summary, validate-artifact, workspace-audit / native-dependency-audit, compare-backends-audit, and validation-summary directly when you only need the individual maintainer-facing artifacts, and the user-facing CLI mirrors the same maintainer-facing renderers, including the workspace audit, the comparison-audit smoke path, and the full packaged-artifact inspection command. The compact compatibility-profile-summary view now also cross-references `verify-release-bundle` and `release-summary`, the release notes artifact now also names `release-summary`, the release-notes summary now also names `Packaged-artifact summary`, `Artifact summary`, and `Artifact validation` alongside `artifact-summary / artifact-posture-summary`, the release summary now also names `backend-matrix-summary` and `validation-report-summary`, the frame-policy summary exposes the shared request-frame posture directly, the request-policy summary exposes the shared request-semantics posture directly and the reproducibility guide now names the matching `request-semantics-summary` alias, the backend-matrix summary now also points to `validation-report-summary` and `verify-release-bundle`, the API stability summary now also points to `verify-release-bundle`, the validation-report summary now also points to `verify-release-bundle` and `release-notes-summary`, the artifact summary now also points to `release-checklist-summary`, `verify-release-bundle`, and `workspace-audit`, and now also surfaces the packaged-artifact output-support posture alongside the profile and storage/reconstruction posture, and the release-checklist summary now also points directly to `validation-report-summary`, `artifact-summary`, and `api-stability-summary` in addition to the compact release-summary view and the staged-bundle verifier, so maintainers can jump between the compact release surfaces without changing the underlying compatibility profile:
...
cargo run -q -p pleiades-validate -- utc-convenience-policy-summary
...
The staged bundle now also carries the catalog inventory, compatibility-caveats, reference-holdout-overlap, production-generation summary, production-generation source summary, release-profile-identifiers summary, release-house-system-canonical-names summary, release-house-validation summary, release-ayanamsa-canonical-names summary, request-policy summary, request-semantics summary, time-scale policy summary, delta-t policy summary, workspace audit summary, native-dependency audit summary, artifact summary, packaged-artifact binary, packaged-artifact checksum sidecar, packaged-frame-treatment summary, lunar-theory-source-family-summary, and lunar-theory-source-selection-summary files alongside the core compatibility-profile, release-notes, release-summary, checklist, backend-matrix, API-stability, comparison, validation, benchmark, and manifest artifacts.

If you broaden a source corpus beyond the checked-in fixtures, keep the provenance contract explicit in the same release-facing files: source origin, license/redistribution posture, frame, time scale, column/channel schema, source revision, generation command, checksums, and the evidence class (reference, hold-out, boundary overlay, or provenance-only). The checked-in JPL provenance summaries now surface that evidence class directly, and the production-generation source summary now also names the license posture alongside redistribution, so release bundles do not have to infer it from surrounding prose.

The generated `release-checklist.txt` now also embeds the canonical `bundle-release` and `verify-release-bundle` commands plus a pointer back to this guide, and it now carries the broader source-corpus provenance contract reminder, while `source-corpus-summary.txt` makes the source-corpus posture explicit in the staged bundle and `release-checklist-summary.txt` provides a compact audit view for quick release review, so the bundle stays self-describing for maintainers.
...
# or: cargo run -q -p pleiades-validate -- verify-release-bundle --output /tmp/pleiades-release
```
