...
The validation tool can also render the full compatibility profile, the compact profile summary, verify the release compatibility profile against the canonical catalogs, render the release notes, release-notes summary, release checklist, release-checklist summary, release summary, API stability summary, backend matrix summary, artifact summary, validate-artifact, and validation summary directly when you only need the individual maintainer-facing artifacts, and the user-facing CLI mirrors the same maintainer-facing renderers, including the full packaged-artifact inspection command. The compact compatibility-profile-summary view now also cross-references `verify-release-bundle` and `release-summary`, the release notes artifact now also names `release-summary`, the release summary now also names `backend-matrix-summary` and `validation-report-summary`, the backend-matrix summary now also points to `validation-report-summary` and `verify-release-bundle`, the API stability summary now also points to `verify-release-bundle`, the validation-report summary now also points to `verify-release-bundle`, the artifact summary now also points to `release-checklist-summary` and `verify-release-bundle`, and the release-checklist summary now also points directly to `validation-report-summary`, `artifact-summary`, and `api-stability-summary` in addition to the compact release-summary view and the staged-bundle verifier, so maintainers can jump between the compact release surfaces without changing the underlying compatibility profile:
