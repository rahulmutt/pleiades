...
That command rewrites the checked-in `VSOP87B.*.bin` artifacts from the matching vendored `VSOP87B.*` source files, and the crate tests assert the regenerated bytes match the committed blobs.
