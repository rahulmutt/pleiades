...
- Typed lookup helpers now exist for source identifier, model name, and family label, so callers that want to resolve the current baseline through a more explicit key can do so without relying on a single generic matcher.
