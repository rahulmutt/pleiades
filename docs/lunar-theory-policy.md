...
- A compact `lunar_theory_source_summary()` helper and `lunar_theory_source_summary_for_report()` formatter expose the same source-selection record as a shorter release-facing provenance line when reports do not need the full specification string; the compact line now also carries the current validation window so the evidence span stays visible without opening the longer specification text. The structured source-family enum is carried directly on the summary record so future source-backed variants can branch on typed data instead of reparsing labels.
