...
Batch callers should use `validate_requests_against_metadata()` on slices before dispatch so batch support and per-request policy failures surface with the same explicit contract as single-request preflight. That helper keeps the per-request time-scale labels intact, and observer-bearing failures still point at the exact slice entry that triggered the rejection.
