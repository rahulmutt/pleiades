...
- [`pleiades-core::ChartRequest`](../crates/pleiades-core/src/chart.rs): mirrors the same explicit conversion helpers for chart assembly and adds `with_instant_time_scale_offset_checked()`, `validate_time_scale_conversion()`, plus `validate_house_observer_policy()` so the façade can preflight request shape before dispatch.
...
- [`pleiades-backend::EphemerisRequest`](../crates/pleiades-backend/src/lib.rs): exposes the direct-backend conversion helpers, `with_instant_time_scale_offset_checked()`, and `validate_time_scale_conversion()` / `validate_against_metadata()` / `validate_requests_against_metadata()` so backend callers can check the same explicit contract before dispatch, whether they are validating one request or a batch.
...
- [`pleiades-cli chart`](../crates/pleiades-cli/src/main.rs): provides the CLI-facing `--tt`, `--tdb`, `--utc`, `--ut1`, `--tt-offset-seconds`, `--tdb-offset-seconds`, `--tdb-from-tt-offset-seconds`, and `--tt-from-tdb-offset-seconds` flags so the same caller-supplied policy can be exercised from the command line. The parser rejects repeated offset flags and alias collisions in either order so the explicit conversion choice cannot silently fall back to last-write-wins parsing.
...
- `pleiades-cli chart` mirrors the same explicit policy with `--utc`, `--ut1`, `--tt`, `--tdb`, and the offset flags that feed the typed retagging helpers. The CLI now also accepts the explicit `--tdb-from-tt-offset-seconds` alias for TT-tagged chart instants, alongside the broader `--tdb-offset-seconds` spelling.
