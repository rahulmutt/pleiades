...
| `pleiades-houses::HouseRequest` | house-observer requests only | not a time-scale conversion surface | uses the observer for house calculations, and `HouseRequest::validate()` preflights the observer latitude, any obliquity override, and topocentric elevation before calculation; it does not infer body-position time conversion |
...
- `pleiades-houses::HouseRequest` is not a body-position time-conversion surface; it uses the observer only for house calculations.
