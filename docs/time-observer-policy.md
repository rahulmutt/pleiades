...
- The shared validation and release report layers reuse `pleiades-backend::request_policy_summary_for_report()` so the compact summary wording for the time-scale, observer, apparentness, and frame policy stays anchored to one typed backend record instead of being rebuilt separately in each reporting crate.
