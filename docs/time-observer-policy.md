...
- [`pleiades-cli chart`](../crates/pleiades-cli/src/main.rs): provides the CLI-facing `--tt`, `--tdb`, `--utc`, `--ut1`, `--tt-offset-seconds`, `--tdb-offset-seconds`, `--tdb-from-tt-offset-seconds`, and `--tt-from-tdb-offset-seconds` flags so the same caller-supplied policy can be exercised from the command line.
