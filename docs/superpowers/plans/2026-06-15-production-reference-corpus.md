...
**Order matters:** `--emit-slices` now requires `fixture_golden.csv` to already exist in the out-dir (it is not backend-generated) and stamps its manifest entry automatically. So populate it FIRST:

1. Hand-populate `crates/pleiades-jpl/data/corpus/fixture_golden.csv` from the existing trusted Horizons fixtures. To give the fixture-golden cross-check real teeth, pick fixture epochs that also fall on a backbone/boundary epoch generated by the spec (or add those epochs to `boundary_epochs()`), so `cross_check_fixture_golden` finds overlap.
2. Then regenerate the 4 backend slices + the complete 5-entry manifest in one command:

```bash
cargo run -p pleiades-cli -- generate-spk-corpus /path/to/de440.bsp --emit-slices crates/pleiades-jpl/data/corpus
```

`emit_slices` reads `fixture_golden.csv`, computes its `corpus_checksum64`, and appends its `slice fixture_golden …` line to `manifest.txt` — no hand-editing of the manifest is needed. If `fixture_golden.csv` is missing it fails with an actionable message.
